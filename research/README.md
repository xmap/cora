...
Staging notes, not published docs. This tree holds what CORA can learn about real facilities from public sources, ahead of (or alongside) modeling them as deployments. It is the upstream fact-gathering that feeds `docs/deployments/<id>/` and the descriptors under `deployments/`, never the other way around. Nothing here publishes to the docs site.
