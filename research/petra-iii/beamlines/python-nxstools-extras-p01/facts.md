# Extracted facts: python-nxstools-extras-p01

Machine-extracted candidate facts for `python-nxstools-extras-p01` (facility `petra-iii`). Candidates only; confirm every row before modeling. Source: DESY OnlineXML (the beamline's online_*.xml Tango device registry).
