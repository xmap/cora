...
| Monochromator | Monochromator | BLISS `dcm` (BM23Mono, `mono/bm23mono.yml`) | crystals Si111 (d 3.13467) / Si333 (1.04489) / Si311 (1.63702) / Si511 (1.04514); real bragg `mbragg` -> `cbragg` (corrected); change `mty` (iced231); energy `Emono` (keV) | oh | optics | yes |
| KBMirror | Mirror | BLISS `kb` (KbController, `controllers/kb.yml`) | hfocus (benders kbh1/kbh2, offset kbho) + vfocus (benders kbv1/kbv2, offset kbvo) on iced232 / `nf8752_kb`; KbMirrorCalcMotor kbvry/kbvtz; focus feedback on diagbpm | oh | optics | yes |
| PrimarySlits | Slit | BLISS slit (`slits`, `motors/slit_ps.yml`) | real psh/psr (h), calc pshg/psho; vertical pair | oh | optics | yes |
| BeamViewers | Screen (?) | BLISS `wbv` / `mirrorbv` / `monobv` / `diagbpm` (EBV / BM23 viewers, `controllers/beamviewer.yml`); cameras `d23/limaccds/bas_wb` | white-beam / mirror / mono / diagnostic beam viewers; diagbpm doubles as KB focus feedback | oh | diagnostics | yes |
