...
So MAX IV is **NOT facility-wide Tier-2-buildable** the way ESRF (Beacon) or PETRA III (gitlab.desy.de) are. BUT it is **partially buildable, beamline-by-beamline**, from two public escape hatches that carry real Tango device handles (`domain/family/member`), confirmed by direct read:

- **`maxiv-science/contrast`** (GitHub, MAX IV's own light-weight DAQ framework, actively maintained, pushed 2026-01) has a `beamlines/` directory with per-beamline device setups for **NanoMAX, CoSAXS, SoftiMAX** (plus a `nanomotionlab`). NanoMAX's `diffraction.py` binds real handles like `b303a-o/opt/mono-xml` (mono), `b303a-o/opt/mir-01-xml` (VFM), `motor/ivu_gap_ctrl/1` (undulator gap); CoSAXS binds `b310A/ctl/*` (~70 device-handle lines). **[verified]**
- **`MaxIV-KitsControls/sardana-biomax`** (GitHub, archived 2022 but public) hardcodes a real **BioMAX** handle: `b311a-o/opt/mono-ener/Position`. **[verified]** A 2022 snapshot, so weaker than `contrast`.

| Beamline | Port | Technique | Source type | Control source (public?) | Source |
| --- | --- | --- | --- | --- | --- |
| NanoMAX | B303A | hard X-ray nanoprobe (imaging, diffraction, ptychography) | undulator (R3) | **yes** (`contrast/beamlines/nanomax`, real handles) | [contrast](https://github.com/maxiv-science/contrast) |
| CoSAXS | B310A | coherent / small-angle X-ray scattering (SAXS/WAXS) | undulator (R3) | **yes** (`contrast/beamlines/cosaxs`, real handles) | [contrast](https://github.com/maxiv-science/contrast) |
| SoftiMAX | B316A | soft X-ray microscopy (STXM, ptychography, coherent imaging) | undulator (R3) | **partial** (`contrast/beamlines/softimax`, different idiom, needs deeper read) | [contrast](https://github.com/maxiv-science/contrast) |
| BioMAX | B311A | macromolecular crystallography (MX) | undulator (R3) | **partial** (`sardana-biomax`, archived 2022, ≥1 real handle) | [sardana-biomax](https://github.com/MaxIV-KitsControls/sardana-biomax) |
...
**Strongest next picks given CORA's growth ladder.** The CORA-relevant tomography beamlines (ForMAX, TomoWISE) have **no public device source**, so they are staff-question deployments, not Tier-2 device passes. The beamlines that ARE Tier-2-buildable from public source are **NanoMAX, CoSAXS, SoftiMAX** (via `contrast`) and **BioMAX** (via the 2022 `sardana-biomax`); none is tomography, but NanoMAX (nano-imaging / ptychography) is the closest to CORA's imaging ladder and is the strongest single candidate if a MAX IV device pass is wanted before ForMAX/TomoWISE staff facts arrive.
...
**Why a full device model is only partially integrity-buildable from public source.** The canonical per-beamline device list with real handles is on the **firewalled internal GitLab**, so a facility-wide Tier-2 pass is not buildable the way ESRF / PETRA III were. BUT two public sources carry real `domain/family/member` Tango handles for specific beamlines: `contrast/beamlines/{nanomax,cosaxs,softimax}` (live, the strongest source) and the archived `sardana-biomax` (one confirmed BioMAX handle, 2022). For those four beamlines a device pass IS buildable from public source; for all others (including the CORA-relevant ForMAX and TomoWISE) device topology must be routed to MAX IV KITS staff. Inference from the generic shared Tango/Sardana tooling is not source.
