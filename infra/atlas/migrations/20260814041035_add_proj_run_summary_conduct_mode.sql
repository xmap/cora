...
-- legitimate state for this axis. Every Run genesis to date really was
-- Conducted (nothing in the codebase constructs a Recorded-mode Run yet),
-- so backfilling existing rows to 'Conducted' is a true fact, not a
-- guess. Additive + forward-only; immutable after genesis by aggregate-
-- level invariant, same as pinned_calibration_ids.
