...
interaction to ours: it pairs recorded notebook history with interactive
techniques for comparing and replaying many recorded versions. Its replay
navigates and re-creates recorded versions rather than folding an immutable
event stream, so it does not certify that a reconstructed version matches what
happened, and it targets a single analyst's local notebook rather than an
