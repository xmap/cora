...
never rendered to the user. We differ in that we surface a per-cursor,
content-addressed fidelity badge that makes the faithfulness of the replay a
...
renders the authenticity of recorded media, and its recent extension to
machine-learning pipelines~\cite{atlas2025}, which performs a content-addressed
hash-versus-reference check but does so at a programmatic service layer over
discrete static artifacts, with no visualization and no notion of replayed
