...
`Mask` is now a shared catalog Family (earned in once both 2-BM and TomoWISE used it). The two microscopes compose the shared `Microscope` / `Optics` catalog Assemblies (Housing-anchored), so there is no loose `Microscope` family any more. The remaining families not yet in the catalog are bound loosely: `HeatAbsorber` is a passive beam-path element earned into the catalog when a confirmed device needs it (the beam-path stop tier is tracked below), and `SlipRing` is the rotation-stage electrical feedthrough, a Positioner component rather than a standalone Asset (so it stays loose by nature, not pending graduation). The nano manipulator reuses the same `TiltStage`, `LinearStage`, `RotaryStage`, and `Table` Families as the micro endstation, so it needs no nano-specific Family.
