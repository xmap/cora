...
- [Detector](detector.md): the RIXS and XES dispersive spectrometer arms bound to the catalog `SpectrometerArm` (graduated, `RIXS-1`); the scattered-beam polarimeter bound to the catalog `PolarizationAnalyzer` (graduated, `POL-2`); and the two Andor CCDs bound to `Camera` (`DET-1`).
