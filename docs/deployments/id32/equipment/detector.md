...
The detection side coins no new Family. The CCDs are area detectors and bind `Camera`; the dispersive arms reuse the loose `SpectrometerArm` SIX coined; the polarimeter reuses the loose `PolarizationAnalyzer`. The genuinely new modelling at ID32 is not a device class but the rule-of-three those loose families reach, held here and routed to dedicated graduation PRs.

## Families

Reused from the catalog: `Camera` (the two Andor CCDs). Reused loose, held at their rule-of-three: `SpectrometerArm` (the RIXS and XES arms, `RIXS-1`) and `PolarizationAnalyzer` (the RIXS polarimeter, `POL-2`). The diffractometer and reciprocal-space axis the arms read against live on the [Sample](sample.md) side. See [Inventory](../inventory.md) for the Asset tree and [Model](../model.md) for the graduation plan.
