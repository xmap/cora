...
Reused from the catalog: `Camera` (the Eiger and flag cameras) and `GenericProbe` (the scalers). The beam-position and intensity monitors are bound to a loose `BeamPositionMonitor` Family, already loose at 2-BM, so POLAR is the second deployment to reference it and the closest of POLAR's loose Families to graduating (see [Model](../model.md#loose-family-graduation)). Whether each monitor is a true position monitor or an intensity (I0) normalizer is `BPM-1`; the Vortex classification is `DET-2`. The detector camera model is `DET-1`. See [Inventory](../inventory.md) for the Asset tree.
