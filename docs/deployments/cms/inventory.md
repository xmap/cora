...
| `BeamPositionMonitor` | `Device` | PositionMonitor | cms-endstation | BIM5 four-quadrant diamond-diode BPM, `XF:11BMB-BI{BPM:1}` (DIAG-1) |
| `SupportTable` | `Device` | Table | cms-endstation | endstation modular support table on three jacks, `XF:11BMB-ES{Tbl}` |

Families reused from the catalog: `Monochromator`, `Mirror`, `Slit`, `Filter`, `PseudoAxis`, `Goniometer`, `TiltStage`, `LinearStage`, `TemperatureController`, `Camera`, `BeamStop`, `FluxMonitor`, `Table`, and the graduated `PositionMonitor` (presenting `Sensor`, distinct from `FluxMonitor` by measuring beam position rather than flux; the per-Asset channel map stays DIAG-1). Loose families reused from siblings: `StorageRing` (supply). No new family is coined and nothing graduates.
