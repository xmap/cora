...
Each device binds a catalog [Family](../../../catalog/families.md) and a verified EPICS PV; none binds a vendor Model (part numbers are not in the public config). FMX graduates nothing: it reuses the i03 MX vocabulary (the graduated `Goniometer`, `Camera`, `Transfocator`, and the graduated catalog `BeamPositionMonitor` presenting `Sensor`, distinct from `FluxMonitor` by measuring beam position rather than flux); the loose family is the held `Backlight`, and the robot is a Positioner Asset with no Family.
