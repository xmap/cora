...
| DIAG-1 | Nice-to-have | The beam-position channel map (the Prosilica BPM cameras, the sector XBPM); the `BeamPositionMonitor` Family is graduated (catalog, presenting `Sensor`), only the per-Asset channel map stays pending. | Read-only beam-position (graduated catalog `BeamPositionMonitor`) probes; channel map blank. | The BeamPositionMonitor bindings. |
