...
Each device the profile exposes binds a catalog [Family](../../../catalog/families.md) and a verified EPICS PV; the shutters, the Eiger, and the motion controller are carried confirm-only where the profile omits them. AMX graduates nothing: as FMX's sibling it reuses the i03 / FMX MX vocabulary (the graduated `Goniometer`, `Camera`, and the graduated catalog `PositionMonitor` presenting `Sensor`, distinct from `FluxMonitor` by measuring beam position rather than flux); the robot is a Positioner Asset with no Family.
