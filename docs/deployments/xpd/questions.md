...
| DIAG-1 | Nice-to-have | The ion-chamber and quad-electrometer channel map (which channel is I0); the `BeamPositionMonitor` Family is settled (graduated catalog Family presenting `Sensor`), so only this per-Asset channel map stays open. | Read-only flux counters (`FluxMonitor`), the graduated catalog `BeamPositionMonitor`; channel map blank. | The IonChamber / QuadElectrometer bindings. |
