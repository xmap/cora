...
| `Backlight` | `Backlight` | Positioner | `BL19I-EA-IOC-12:` | sample backlight, in / out illumination for OAV centring (EH2); binds the catalog Family (DET-1) |
...
- **The backlight binds the catalog `Backlight` Family.** The in / out illumination for OAV centring binds the `Backlight` Family that i03 introduced, the illumination affordance since graduated across the MX / imaging fleet (after i03 / i24 / fmx), so i19 is a further consumer (DET-1).
...
The detection side coins nothing. The Eiger is `Camera` reuse; both beamstops are `BeamStop` reuse; all three trigger boxes are `TimingController` reuse with the PandA / Zebra split living in a bound Model rather than in a new Family. The backlight binds the catalog `Backlight` family rather than minting a fresh one, the illumination affordance graduated across the MX / imaging fleet (DET-1). No new Family is earned on the detector side and nothing graduates here; the catalog is unchanged.
