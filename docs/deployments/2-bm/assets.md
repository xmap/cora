...
| `Mirror` | `Translatable`, `Homeable`, `Limitable` (vertical-deflecting optic; Y/pitch positioning plus a coating-stripe selector) |
| `Monochromator` | `Translatable`, `Homeable`, `Limitable` (crystal positioning selects the energy; insertable, bypassed in pink beam) |
| `Slit` | `Translatable`, `Homeable`, `Limitable` (four independent blades define the aperture) |
| `Filter` | `Indexable`, `Attenuable` (discrete absorber-foil selection that attenuates the beam) |
...
| `kohzu_cyat_070` | Kohzu | `CYAT-070` | `LinearStage` | `SampleTop_X`, `SampleTop_Z` |
...
Devices that physically exist at 2-BM but are not yet registered as CORA Assets; each carries `new: true` in the 2-BM descriptor (`deployments/2-bm/beamline.yaml`). The front-end optics (Mirror, Monochromator, ConditioningSlit, SampleSlit, Filter) are now modelled and driven by `FrontEndDrive` (see the Inventory above); what remains is the beam-position diagnostic, the timing box, the broader sample-stage motor band, and the IOC-hosted devices. `BeamPositionMonitor` is a diagnostic, not a motor: the descriptor records no PV or controller for it.
