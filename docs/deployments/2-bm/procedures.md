...
| `energy_setting` | the energy-tracking facets (`Monochromator` Bragg arms + M2 offset, `SampleSlit` vertical pair) |
...
A sibling coordinated operation, `beam_mode_change`, is design-locked but deferred. 2-BM runs two beam modes (the monochromator inserted vs bypassed, see [Beam modes](#beam-modes-mono-pink)), and switching between them is one coordinated multi-device move (DMM in/out, the mirror coating stripe with its table-X, downstream tracking) of the same Method + Procedure shape as `energy_setting`, with the target mode (mono or pink) as a parameter rather than two verb-first kinds, paired with a `BeamModeChange` Decision for the operator's choice. It is gated on the staff facts `MODE-1` / `MODE-2` / `MODE-3` and `MIRROR-1` before it can carry real positions or drive hardware. Aligning the beam within each mode is the related deferred [`beam_alignment` family](#beam-alignment-item_012).
