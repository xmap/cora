...
| --- | --- | --- | --- | --- |
| `dark_baseline` | `acquisition` | `StationShutter` + detector | `repetitions`, `dwell` | Conductible today |
| `flat_baseline` | `acquisition` | `StationShutter` + detector | `repetitions`, `dwell` | Conductible today |
...
Both baselines are subject-less calibration captures that feed reconstruction, and both reuse the registered `collect` action body (acquire a frame stack, poll until done), so they are conductible today. The pixel-wise baseline math (mean / std) is downstream data reduction, not a recipe step (per the catalog convention that reduction lives in pipelines, not CORA); the captured stack becomes a baseline [Dataset](datasets.md). These same captures are also modeled as subject-less calibration [Runs](runs.md): a recipe is the as-data form of the capture sequence.
...
The setpoint positions shown are illustrative curve outputs and are **provisional**: the per-energy curves await the real saved-position table from 2-BM staff. The curve-interpolation runtime (`eval_lookup_table`) exists, but a live free-keV recipe is still blocked on two things: the saved per-energy table is not populated, and the Plan.wiring-backed constituent resolver that would let the recipe address the five facets as `pseudoaxis://` constituents is deferred. So a v1 recipe encodes **one energy's resolved positions** as literal setpoints, one recipe per saved energy. The saved menu here is the **Mono** menu (13.374 / 13.574 / 18.0 / 20.0 / 25.0 / 25.584 keV); `energy_setting` as written is the mono-mode recipe, and pink-mode energy selection (30 / 40 / 50 / 60 keV) is deferred with the [beam-mode work](questions.md#beam-mode-mono-pink) (`MODE-3` / `MIRROR-1`). `energy_kev` is recorded but does not drive live position computation at v1; once the saved table lands and the resolver ships, the runtime can interpolate an in-between energy (for example 22 keV, between the saved 20.0 and 25.0) rather than only the menu points.
