...
*The 2-BM detector: the stage that records the transmitted beam. It presents the `Detector` Role, realized by the cross-facility `Microscope` Assembly (scintillator, objective optics, camera) materialized here as the `microscope_at_2bm` Fixture.*

The detector sits about 55 m from the source in the 2-BM experiment hutch (Enclosure `2-BM-B`), and records the transmitted beam indirectly: a LuAG scintillator converts the X-rays to visible light, three selectable objectives on a sliding ball-screw selector magnify the image, and a FLIR Oryx camera captures it, all carried on a propagation-distance rail that sets the sample-to-detector distance. That lens-coupled scintillator design is the "microscope" detector type, which CORA models as the `Microscope` Assembly binding eight Assets across six slots over a reusable `Optics` sub-assembly. (A second, higher-resolution Oryx camera and its `Camera_Selector` are also modelled; only the 31 MP sensor settings remain pending, see [Open items](#open-items).) The whole unit is driven by the [BCDA-APS MCTOptics IOC](https://github.com/BCDA-APS/tomo-bits/blob/main/src/tomo_instrument/devices/mct_optics.py) (MCTOptics is the IOC process name, not the CORA model name). This page explains how CORA models it.
