...
| ENERGY-4 | `Nice-to-have` | Do you ever need an energy that is not in the configured saved set (for example 22 keV)? CORA's continuous curves could interpolate to off-list energies, which the discrete saved store cannot; we want to know if that capability is useful before relying on it. | discrete configured energies today; off-list interpolation possible but unconfirmed | not yet | [Energy-tracking optic axes](assets.md#energy-tracking-optic-axes) |
