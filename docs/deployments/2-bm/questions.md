...
| DET-11 | `Blocks-go-live` | When you do the detector "resolution" alignment, the step that sharpens the image on a resolution target (a Siemens star or similar): which motor do you actually turn? The stage that moves the whole microscope along the beam to set the sample-to-detector distance (`PropagationDistance`, `2bmbAERO:m1`), or the microscope's per-lens objective focus (`2bmb:m2`/`m3`/`m4`, the MCTOptics `LENS0/1/2_FOCUS`)? CORA currently has this step adjusting the propagation-distance rail. If it is really the per-lens objective focus, that focus is MCTOptics-owned (DET-2), so CORA would drive it through MCTOptics rather than as its own stage, and may need to register it as a device it does not model yet. (This is separate from the `focus` step, which moves the sample stage `SampleTop_Z` for depth of field.) | resolution step adjusts `PropagationDistance` (`2bmbAERO:m1`) | yes | [Procedures](procedures.md) |
