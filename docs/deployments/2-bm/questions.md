...
| ID | Priority | Question | CORA assumes | Already done? | Resolves |
| --- | --- | --- | --- | --- | --- |
| BLEPS-4 | `Blocks-go-live` | What EPICS record type do the BLEPS flag PVs use: `bi` / `mbbi`, or a numeric `longin` / `ai`? The distinction decides whether a reader sees a number or a text label. An enum record returns its state label (`"TRIP"`, `"OK"`) rather than 0 or 1, and CORA will not guess at label vocabularies, so on an enum beamline it currently records nothing and says so in its log. `bleps.substitutions` states the record types; a single `caget 2bmBLEPS:BLEPS:FLOW2_BELOW_SET_POINT_TRIP` pasted verbatim answers it just as well. | numeric flags (0 / 1), which is what the reader parses today | not yet | [Equipment protection](operations.md#equipment-protection) |
