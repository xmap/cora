...
| `EnergyDispersiveSpectrometer` | `EnergyDispersiveSpectrometer` (loose) | Sensor | germanium energy-dispersive detector; a per-photon energy spectrum |
...
- **Energy-dispersive diffraction (germanium detector).** A germanium detector records the energy of each absorbed photon, an MCA spectrum transformed to scattering vector by Bragg's Law. It presents the Sensor Role (a short-vector Reading). What is open is whether it is the same physical detector as the fluorescence MCA, and whether it is a new Family or a deployment-local Sensor device (DET-1).

## Families

The imaging path reuses `Scintillator` and `Camera`. The two point and spectral detectors are carried as loose families (`Photodiode`, `EnergyDispersiveSpectrometer`) that present the existing Sensor Role and render as plain text; whether either is earned into the catalog or stays a deployment-local Sensor device is settled when staff confirm the devices and the naming review weighs in. This is the same loose-binding pattern 2-BM uses for its uncatalogued `Diagnostic` and `BeamPositionMonitor` Sensor devices.
