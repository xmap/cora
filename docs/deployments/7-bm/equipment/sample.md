...
- **Does CORA command the setpoints?** The flow controllers and the electronic air regulator are settable, not just readable. Whether CORA commands the setpoint (versus reading it back only), and whether a settable-actuator affordance or a `FlowController` Family is earned, is the load-bearing question here (FLOW-1). It is also the natural rule-of-three sibling that would decide a settable-actuator shape across the deployments.
