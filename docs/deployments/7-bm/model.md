...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | 7-BM reuses existing Families including `EnergyDispersiveSpectrometer` (graduated once 2-ID and 7-BM shared it); it carries three loose design-intent families (`Chopper`, `FlowController`, `Photodiode`) that render as plain text until earned |
