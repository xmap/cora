...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | **no Family graduated here.** MFX reuses catalog Families, including the graduated `Transfocator` (a CRL focusing optic), and carries loose families (`FluxMonitor`, `Diagnostic`, `Laser` reused; `EmissionSpectrometer` new and loose) |
