...
| Source | An undulator on the ESRF_Undulator device server |
...
| --- | --- | --- |
| Optics (`id28-optics`) | Yes | The undulator, the incident-energy pseudo-axis, the high-resolution backscattering monochromator, the HFM / VFM focusing mirrors, the oh2 beam-position monitor |
| IXS endstation (`id28-eh1`) | Yes | The multi-analyzer spectrometer arm (loose `SpectrometerArm`), the sample stage, the sample-temperature cryostats, the detectors |
...
- **No new family, nothing graduates here.** The backscattering mono binds `Monochromator`, the benders `Mirror`, the detectors `Camera`, the cryostats `TemperatureController`; the catalog is unchanged.
...
- [Source](beamline.md): the generated device walk: the storage-ring state, the undulator, the incident-energy pseudo-axis, the backscattering monochromator, the focusing mirrors, and the beam-position monitor.
- [Sample](equipment/sample.md): the IXS sample stage and the sample-temperature cryostats.
