...
| --- | --- | --- |
| `SpectrometerArm` | `SpectrometerArm` (loose) | the IXS multi-analyzer spectrometer: a two-theta arm (BLISS `tth_multilayer`, `TwoThetaMultilayer`) carrying an array of inclined analyzer crystals (`a2_inca` / `a3_inca` / `a4_inca`, each chi / th); a further consumer of the loose family, held (`RIXS-1`, `IXS-1`) |
| `Detector` | `Camera` | the Basler / PCO counting and imaging detectors at the spectrometer endstation (BLISS lima `basler` / `pco`); the per-analyzer IXS counters are `DET-1` |
...
The multi-analyzer arm is the signature ID28 instrument, and it binds the loose `SpectrometerArm` Family. This is a **further consumer** of the family SIX coined and ID32 brought to a rule-of-three (SIX RIXS arm + ID32 RIXS arm + ID32 XES arm): ID28's arm is the same anatomy, an arm that **positions** an energy-dispersing element (here a crystal array, at SIX / ID32 a grating) and **carries** detectors, presenting the `Positioner` Role, which is why it never fit the point-Sensor families. The graduation is **already pending** from ID32 (`RIXS-1`), deferred to a dedicated catalog PR; ID28 reinforces it and stays held, so this scaffold makes no catalog change.

The arm carries several inclined analyzer crystals (`a2` / `a3` / `a4`), each with its own chi / th. The first cut carries the array as a per-Asset setting on the one `SpectrometerArm` Asset; promoting each crystal to a child Asset via `parent_id` is the nested-component-identity convention (the IXS 10-ID diced-crystal question is the sibling), so ID28 flags `IXS-1` rather than asserting it.

## Why no new family

The detection side coins no new Family. The arm reuses the loose `SpectrometerArm`; the Basler / PCO detectors reuse the catalog `Camera`. The genuinely new modelling at ID28 is not a device class but the further sighting of the loose `SpectrometerArm`, held and routed to the graduation PR already pending from ID32.

## Families

Reused loose, held at its further sighting: `SpectrometerArm` (the multi-analyzer arm, `RIXS-1`, `IXS-1`). Reused from the catalog: `Camera` (the Basler / PCO detectors). The backscattering monochromator and incident-energy axis the arm reads against live on the [Source](../beamline.md) walk; the sample stage and cryostats on the [Sample](sample.md) side. See [Inventory](../inventory.md) for the Asset tree and [Model](../model.md) for the graduation plan.
