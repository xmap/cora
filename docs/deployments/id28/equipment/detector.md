...
| --- | --- | --- |
| `SpectrometerArm` | `SpectrometerArm` (loose) | the IXS multi-analyzer spectrometer: a two-theta arm (BLISS `tth_multilayer`, `TwoThetaMultilayer`, `tth` over the `tthm` rail) carrying an array of inclined analyzer crystals (`inca` controllers `a2_inca` / `a3_inca` / `a4_inca`, each chi / th, with analyzer cylinder slits `a1h..a9h` / `a1v..a9v`); a further consumer of the loose family, held (`RIXS-1`, `IXS-1`) |
