...
This is the reinforcement point of CHX as a CORA exercise: a coherence beamline with three area detectors, a detector positioner, and a beamstop that needs **no new Family**. The Eiger detectors reuse `Camera` (the Diamond Eiger-to-Camera precedent, also used at [SRX](../../srx/detector.md)); the flux counter reuses `FluxMonitor` (graduated in #353); the beamstop reuses `BeamStop`; the occasional fluorescence detector reuses `EnergyDispersiveSpectrometer` (graduated in #345). The `BeamViewingCamera` (an on-axis Prosilica OAV; x-ray-eye Prosilicas and a PointGrey serve alignment views) is another `Camera`. The live set is CAM-1.
