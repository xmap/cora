...
| --- | --- | --- | --- |
| `IonChambers` | FluxMonitor (loose) | `XF:06BM-BI{EM:1}EM180:` | I0 (incident), It (transmitted), Ir (reference) currents |
...
These ion chambers are the **primary measurement detector** here, not auxiliary flux monitors, but they are the same device kind as the `FluxMonitor` loose family the Diamond beamlines (i03/i15-1/i22) carry for their ion chambers. CORA reuses `FluxMonitor` rather than coining a synonym; whether ion chambers earn a dedicated Family (given how central they are to spectroscopy) is the open cross-deployment question (DIAG-1 / FLUX-1).
