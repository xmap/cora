...
- **No new Family.** BMM reuses existing catalog Families and loose families already carried by siblings: the ion chambers reuse `FluxMonitor` (held, FLUX-1), the fluorescence detector the catalog `EnergyDispersiveSpectrometer`, the diagnostic screens `Screen` (held, FLAG-1). The sample wheel reuses `RotaryStage`; whether a sample-changer Family is earned across BMM and the Diamond robots is open (WHEEL-1).
