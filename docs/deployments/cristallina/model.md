...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | **none graduated.** Cristallina reuses catalog Families and the graduated `Diffractometer` Assembly and `TemperatureController` Family; loose families (`Magnet`, `Laser`, `Diagnostic`, `FluxMonitor`) reused |
...
The DilSc sample environment is a dilution refrigerator with a 3-axis vector superconducting magnet (an Oxford Mercury iPS, field limits X,Y = ±0.6 T and Z = ±5.2 T). The magnet binds the **loose `Magnet`** family, which is already held at the rule-of-three: its three consumers are 4-ID, i10-1, and ESRF ID32 (the 9 T XMCD magnet). Cristallina is the **fourth consumer**, which reinforces the case for graduation, but the graduation stays deferred to its dedicated gated PR rather than being slipped in here (MAG-1). This follows the ID32 posture exactly: bind loose, note the consumer count, do not graduate outside the gated PR. The LakeShore 372 thermometry / heater binds the **graduated `TemperatureController`** Family (presents the Regulator Role), the ID32 VTI precedent. The vector geometry (three independently-ramped field axes) is a richer setting than the single-axis magnets, but it is a per-Asset setting, not a Family split, the same way the diffractometer axis counts are.
...
- **The Magnet graduation (MAG-1).** Reinforced as a fourth consumer; deferred to the gated PR.
