...
| DIAG-1 | Nice-to-have | The beam-position channel map (the Prosilica BPM cameras and the sample-positioner cameras); the `BeamPositionMonitor` Family is settled (graduated catalog Family presenting `Sensor`, distinct from `FluxMonitor` by measuring beam position rather than flux). | Read-only beam-position (graduated catalog `BeamPositionMonitor`) probes; channel map blank. | The BeamPositionMonitor bindings. |
