...
!!! warning "First cut, and confirm-pending by intent"
    This scaffold was reverse-engineered from the beamline's own Bluesky instrument repo ([BCDA-APS/8id-bits](https://github.com/BCDA-APS/8id-bits)); the extraction is in [`research/aps/`](https://github.com/xmap/cora/tree/main/research/aps). Like 4-ID it binds the real EPICS control handles, because 8-ID is operational. Every value is carried as `confirm` until 8-ID staff verify it. What CORA needs the team to confirm is on [Open questions](questions.md).
