...
The diffractometer is modelled as a plain device with its circle axis map; the reusable `Assembly(Diffractometer)` is designed but deferred (see [Model](../model.md#deliberately-not-here-yet)). The temperature controllers and the beam-position monitor bind Families that **graduate into the catalog with this deployment** (8-ID is the second independent beamline to use them); the rheometer binds a loose `Rheometer` Family.
...
| --- | --- | --- |
| `Diffractometer_SixCircle` | `Diffractometer` (loose) | six circles mu / eta / chi / phi / nu / delta + x/y/z; confirms the Assembly slot shape (`DIFF-1`) |
