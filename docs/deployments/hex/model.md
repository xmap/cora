...
- **The B / C / D / E hutches (`ENC-1`, `LAYOUT-1`).** HEX is designed for six enclosures (A = FOE, B, C, D, E, F), but B is not erected and C / D / E are future-upgrade shells. Modelling empty enclosures would be noise (the descriptor validates that every device's enclosure ref is declared), so only the operational FOE (`hex-foe`) and F-hutch (`hex-endstation`) are modelled. The satellite-building identity and the per-hutch positions are carried as world-facts (`SAT-1`, `LAYOUT-1`).
