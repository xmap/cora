...
Adding P02 as `PressureCell`'s second consumer crossed the rule-of-three promotion threshold, so the loose-family review records a promote-or-hold note (`hold: graduation-due, PRESSURE-1`), matching the `Magnet` entry P09 and the POLAR-family beamlines carry (its `PhaseRetarder` and `PolarizationAnalyzer` siblings have since graduated to catalog Families). This is the loose-family graduation guard working as designed.
