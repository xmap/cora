...
Neither side coins a Family. The diffractometer reuses `Goniometer`, the cryostat stage and the magnet stages reuse `LinearStage`, the Lakeshores reuse the graduated `TemperatureController`, the pinhole reuses `Aperture`, the point detection reuses `FluxMonitor`, and the polarization-analysis arm binds the graduated `PolarizationAnalyzer` (earned across 4-ID / i10 / ID32 / P09, presenting Positioner). The two magnet devices bind the loose `Magnet`, which is not a catalog Family: a second sighting (after 4-ID), held under review, graduating nothing here (see [Loose families at a second sighting](#loose-families-at-a-second-sighting)). The genuinely new modelling move at i10, polarization as a driven axis, sits on the source side as a reused `PseudoAxis`, not here (see the generated [Source walk](../beamline.md) and [Model](../model.md)).
...
Two families on this page were first seen at 4-ID. `PolarizationAnalyzer` (the RASOR analysis arm, POL-2) has since graduated to a catalog Family across 4-ID / i10 / ID32 / P09, presenting Positioner, so the arm binds the catalog Family. `Magnet` (the i10-1 electromagnet and superconducting magnet, after 4-ID, MAG-1) is still loose, not a catalog Family, at its second sighting, carried as a loose family string here and bound to a real device so the modelling is honest about what the endstation does.

`Magnet` does not graduate at i10. Graduation follows a rule-of-three across deployments, and a second sighting does not meet it; the hold-versus-graduate call is human, and i10 records a hold for it (MAG-1). This mirrors how the fleet carries `StorageRing` loose at i22 until a confirmed device and a naming review settle it. The loose annotation in the tables above now marks only the magnet; the analyzer arm reuses the catalog `PolarizationAnalyzer`, and everything else on this page reuses a catalog Family unchanged.
