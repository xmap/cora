...
- **The analyzer arm is sample-side, and its crystal is implicit.** RASOR's polarization-analysis arm (the PaStage / POLAN arm, `ME01D-MO-POLAN-01`) carries its own analyzer two-theta and detector. It binds the graduated catalog `PolarizationAnalyzer` Family (earned across 4-ID / i10 / ID32 / P09, presenting Positioner, POL-2), and is modelled with the rest of the analyzer mechanics on the [sample](sample.md) side; dodal exposes the arm motors only, so no analyzer-crystal specifics are invented here.
