...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | no new Family added by I22 itself; it reuses existing Families and carries loose design-intent families (`StorageRing`, `Transfocator`, `FluxMonitor`, `FlowController`). `TemperatureController` (the Linkam) was carried loose here too but has since graduated to a catalog Family (presenting the `Regulator` Role) once it reached the rule-of-three at i11 |
