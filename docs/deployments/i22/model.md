...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | no new Family added by I22 itself; it reuses existing Families and carries loose design-intent families (`StorageRing`, `Transfocator`, `FlowController`). `TemperatureController` (the Linkam) was carried loose here too but has since graduated to a catalog Family (presenting the `Regulator` Role) once it reached the rule-of-three at i11; `FluxMonitor` (the I0 / It ion chambers) likewise graduated, presenting the Sensor Role, on the i22/i03/i15-1 rule-of-three |
