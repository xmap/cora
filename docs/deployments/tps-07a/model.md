...
- **The reused loose families.** `StorageRing` (the ring-current monitor) and `PositionMonitor` (the beam-position diagnostic) are bound loose, each already allowlisted from earlier deployments; TPS 07A coins no new loose family.
