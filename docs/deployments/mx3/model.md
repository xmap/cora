...
- **The reused loose families.** `StorageRing` (the ring-current monitor) and `Backlight` (the MD3 backlight) are bound loose, each already allowlisted from earlier deployments; MX3 coins no new loose family. The beam-position monitor binds the graduated catalog `BeamPositionMonitor`, which presents `Sensor`, earned across the wide fleet that shares it, distinct from `FluxMonitor` by measuring beam position rather than flux.
