...
LIX's measurement, small- and wide-angle X-ray scattering, is a science axis the fleet already speaks. The materials-scattering beamlines [SMI](../smi/techniques.md), [CMS](../cms/techniques.md), Diamond [I22](../i22/techniques.md), and APS [9-ID](../9-id/techniques.md) / [12-ID](../12-id/techniques.md) all run small- and wide-angle scattering on the same `Camera` / `FluxMonitor` / `BeamStop` vocabulary. So the scattering hardware and detection are reinforcement, not novelty.
