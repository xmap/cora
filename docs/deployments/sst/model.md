...
- **The `ElectronAnalyzer` graduation candidate.** The HAXPES Scienta SES hemispherical analyzer binds the loose `ElectronAnalyzer` family NSLS-II ESM introduced. SST-HAXPES is the **second** sighting (the same Scienta SES type), so it crosses the promotion threshold. The abstraction is settled (a hemispherical electron analyzer is a distinct, recurring photon-in / electron-out device, not a photon detector), which makes it a clean graduation candidate, the way `GratingMonochromator` (CSX) and `Manipulator` (ESM) graduated at their second sighting. It is held loose and recorded in the promotion register pending the gate-review decision (`ARPES-1`), rather than graduated inside this scaffold PR.
