...
| ID | Priority | Question | CORA assumes | Resolves |
| --- | --- | --- | --- | --- |
| ARPES-1 | Blocks-go-live | The `ElectronAnalyzer` graduation: the HAXPES Scienta SES hemispherical analyzer is a SECOND sighting of the loose `ElectronAnalyzer` family (ESM was the first, the same analyzer type), so it has crossed the promotion threshold. Graduate it into the catalog, or hold it loose? | Held loose, recorded in the promotion register; a graduation candidate pending gate-review. | The ElectronAnalyzer catalog decision. |
