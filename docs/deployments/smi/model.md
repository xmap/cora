...
- **The held loose family.** One device binds a loose family that other deployments also use and that is held for gate-review: the `BeamPositionMonitor` (4-ID, 8-ID, 9-ID), which stays loose pending the sensor fold-vs-promote decision (`DIAG-1`), recorded in the promotion-review register.
