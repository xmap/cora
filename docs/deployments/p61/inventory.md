...
Families reused from the catalog: `LinearStage`, `EnergyDispersiveSpectrometer`. No new family is coined and nothing graduates. The Large Volume Press (P61A), when exposed, would reuse the allowlisted-loose `PressureCell` Family (the P02 / 13-id-d precedent, `PRESS-1`).
