...
| DIAG-1 | Nice-to-have | The Sydor beam-position monitor (per-quadrant currents, beam x / y, a sum-current total flux) and the per-Asset position-versus-intensity split. The `BeamPositionMonitor` Family itself is settled (graduated catalog Family presenting `Sensor`). | A graduated catalog `BeamPositionMonitor` (earned across the wide fleet that shares it); only the per-Asset channel map is pending. | The beam-position-monitor channel map. |
