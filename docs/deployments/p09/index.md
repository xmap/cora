...
- **The 4-ID vocabulary ports cleanly.** P09 reuses the allowlisted-loose `PhaseRetarder`, `PolarizationAnalyzer`, and `Magnet` Families that the APS 4-ID deployment introduced (the second consumer of `PhaseRetarder`, a further consumer of the other two). No new Family is coined.
...
[Model](model.md): the developer's by-kind index, P09's place as a further consumer of the 4-ID polarization / magnetism vocabulary, and the record of what is deliberately deferred.
