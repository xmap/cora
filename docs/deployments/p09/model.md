...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | none changed; P09 reuses the allowlisted-loose `PhaseRetarder` / `PolarizationAnalyzer` / `Magnet` Families + the catalog Families |
...
- **The polarization / magnet detail (`POL-1`, `MAG-1`).** The phase-retarder / analyzer geometry and the 14 T magnet field / control are pending; the Families are the allowlisted-loose 4-ID POLAR ones.
