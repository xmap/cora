...
!!! warning "First cut, and confirm-pending by intent"
    This scaffold was reverse-engineered from the beamline's own Bluesky instrument repo ([BCDA-APS/9id_bits](https://github.com/BCDA-APS/9id_bits)). Like 4-ID and 8-ID it binds the real EPICS control handles, because 9-ID is operational. Every value is carried as `confirm` until 9-ID staff verify it. What CORA needs the team to confirm is on [Open questions](questions.md).
...
[Governance](governance.md): who will act at 9-ID and the trust shape that gates their commands. People and agents are facility principals at the [APS Site](../aps/index.md#safety-and-governance).
