...
- **A RIXS spectrometer arm.** The signature instrument is a meters-long energy-dispersive arm (a bridge truss, an in-arm optics chamber that disperses, and a detector chamber) that swings to a scattering angle and spreads the emitted soft X-rays onto a photon-counting camera. SIX coined the `SpectrometerArm` Family for it; the family has since graduated into the catalog (earned across SIX + ESRF ID32 + ID28).
...
| New device classes | Graduated | `GratingMonochromator` (earned by CSX), `Manipulator` (earned by ESM), and `SpectrometerArm` (earned by ESRF ID32 + ID28) all graduated (see [Model](model.md#new-loose-families)) |
