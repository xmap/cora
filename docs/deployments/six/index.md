...
SIX is **CORA's first soft X-ray beamline**. The 16 hard X-ray deployments before it never exercised this regime, so SIX introduces three device classes new to the catalog, modelled as loose families at n=1 (nothing graduated):

- **A grating monochromator, not a crystal DCM.** The PGM disperses with an interchangeable grating (500 / 1200 / 1800 l/mm) and a premirror at a fixed-focus c-value; the exit slit sets the energy resolution. There is no Bragg crystal, so it binds a loose `GratingMonochromator` Family, not the catalog `Monochromator`.
...
| New device classes | Loose at n=1 | `GratingMonochromator`, `SpectrometerArm`, `Manipulator`: held loose, nothing graduated (see [Model](model.md#new-loose-families)) |
