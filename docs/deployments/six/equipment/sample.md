...
The cryostat manipulator binds the catalog `Manipulator` Family (graduated across SIX + ESM; see [Model](../model.md#new-loose-families)); the endstation mirrors reuse the catalog `Mirror` Family, and the temperature controller the catalog `TemperatureController`.

## The UHV sample stack (2-ID-D)

| Device | Family | Design spec / note |
| --- | --- | --- |
| `SampleManipulator` | `Manipulator` | UHV cryostat manipulator (x/y/z/theta); cryo range and load-lock are `SAMPLE-1` |
