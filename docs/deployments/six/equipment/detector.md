...
The spectrometer arm binds the catalog `SpectrometerArm` Family (the multi-chamber dispersive arm SIX coined, since graduated across SIX + ESRF ID32 + ID28; it presents the `Positioner` Role, distinct from the point-Sensor `EnergyDispersiveSpectrometer`, see [Model](../model.md#new-loose-families)); the camera reuses the catalog `Camera` Family and the counters `FluxMonitor`.

## Detection chain

| Device | Family | Design spec / note |
| --- | --- | --- |
| `RIXSSpectrometer` | `SpectrometerArm` | energy-dispersive arm: bridge truss (`BT:1`) + optics chamber (`3AA:1`) + detector chamber (`DC:1`); the arm angle selects momentum transfer (`RIXS-1`) |
...
Reused from the catalog: `Camera` (the RIXS camera), `Slit` (the detector slit), `FluxMonitor` (the scaler and electrometer), and `SpectrometerArm` (the dispersive arm SIX coined, since graduated across SIX + ESRF ID32 + ID28). Whether the photon-counting camera warrants its own Family (versus `Camera` with settings) is `RIXS-2`; the channel map is `DET-1`. See [Inventory](../inventory.md) for the Asset tree.
