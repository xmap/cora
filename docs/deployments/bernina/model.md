...
| Catalog Family | [`catalog/catalog.yaml`](https://github.com/xmap/cora/blob/main/catalog/catalog.yaml) | **none graduated.** Bernina reuses catalog Families and the graduated `Diffractometer` Assembly; loose families (`Laser`, `Diagnostic`, `FluxMonitor`) reused |
...
- **Femtosecond pump-probe synchronization (LASER-1).** The `eco` `lxt` timing chain and the PSEN arrival-time monitor hold and correct the laser-to-X-ray delay; CORA's single-domain `PartitionRule` cannot express the cross-timing-domain sync. The laser folds (loose `Laser`); the sync is the gap.
