...
- **It reuses the loose families I22 introduced.** The storage-ring state, the flux monitors, and the cryo / thaw actuators reuse `StorageRing`, `FluxMonitor`, and `TemperatureController` rather than coining synonyms; the beam-position monitor reuses 2-BM's loose `Diagnostic` family. The flux monitors at a second deployment were part of the rule-of-three that graduated `FluxMonitor` into the catalog (it presents the Sensor Role).
