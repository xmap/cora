...
Devices bind to a catalog [Family](../../catalog/families.md) wherever one fits. 13-ID, CORA's first extreme-conditions deployment, **introduced** the `PressureCell` Family for the diamond anvil cell (no fleet analog at the time for a high-pressure sample environment); it has since graduated to the catalog, earned across 13-ID and PETRA III P02. Everything else reuses the catalog (see [Model](model.md#the-pressurecell-family-graduated)). The control handles are reconstructed from the GSECARS EPICS support tree at medium confidence; no vendor Models are bound.
...
| `PressureCell` | `Device` | PressureCell | 13-ID-D | the diamond anvil cell: membrane pressure (PACE5000 `13IDD_PACE5000:PC1:`, Regulator), double-sided laser heating (HEAT-1), in-situ P/T metrology; catalog Family (graduated across 13-ID and P02) (HP-1, PRESSURE-1) |
