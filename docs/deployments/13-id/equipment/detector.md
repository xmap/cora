...
| `SampleIllumination` | `Backlight` | the fibre sample illumination (`13IDD:US_IllumOnOff`), the catalog `Backlight` (`DET-1`) |
...
The detector table and 2theta arm decompose into ordinary translation and tilt axes (`LinearStage`, `TiltStage`); the 2theta swing as a derived angle is a `PseudoAxis` transform held back rather than invented, because the only prefix seen for it was a Galil test template (`DET-1`). The fibre illumination binds the catalog `Backlight` (graduated across the MX / imaging fleet). None of these is a new device class.
...
Reused from the catalog: `Camera` (the Eiger2 X 9M and the Pilatus 1M CdTe and Si area detectors), `FluxMonitor` (the ion chambers over the USB-CTR scaler and the DAC photodiode), `EnergyDispersiveSpectrometer` (the XGLab Dante MCA, the 2-ID / 7-BM precedent), `LinearStage` and `TiltStage` (the detector table and 2theta arm), and the `Backlight` (the fibre sample illumination, graduated across the MX / imaging fleet). No new family is coined on the detection axis, and nothing graduates here; the catalog is unchanged. The open detail is the device-to-PV reconstruction, rougher here because the source is the EPICS-native GSECARS support tree rather than a Python device roster (`DET-1`), and the deferred 2theta `PseudoAxis` binding (`DET-1`). See [Inventory](../inventory.md) for the Asset tree and [Model](../model.md) for the no-new-family argument across the whole instrument.
