...
- **EnclosureLookup port.** *(Enclosure BC)* Cross-BC query port consumed by `start_run` and `start_procedure` pre-flight gates. Returns the current permit status for enclosures covering a Run or Procedure's Asset scope; pre-flight rejects with `RunRequiresPermittedEnclosureError` / `ProcedureRequiresPermittedEnclosureError` when no covering Enclosure is found, and `RunEnclosureCoverageMismatchError` / `ProcedureEnclosureCoverageMismatchError` when at least one covering Enclosure is not in `Permitted`. Stage-1 ships an `AlwaysPermitted` stub; the Postgres + EPICS adapters are deferred to first pilot integration.
