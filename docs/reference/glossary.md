...
- **Enclosure.** *(Enclosure BC, 17th BC)* The permit state of an interlock-gated volume (hutch, cabin, cave, vault) whose access-permitted state gates whether dependent Runs and Procedures may start. The anchor is the containing Facility, NOT an equipment Asset: an Enclosure sits within a Federation `Facility` (a Site or Area), referenced by `facility_code`, a space contained in a larger space. Three axes stay separate: institutional geography is the Facility, equipment is the Asset, operational access is the Enclosure. Which Enclosure an Asset operationally sits in is recorded on the Equipment side as `Asset.located_in_enclosure_id`. Observation-axis-only by charter: CORA records the externally-observed permit status, never asserts or commands it. Two orthogonal axes: `permit_status` (`Permitted` / `NotPermitted` / `Unknown`) carries the live observation; `lifecycle` (`Active` / `Decommissioned`) carries terminal retirement. No `Bypassed` state; bypass concerns belong to the upstream interlock system, not the spine.
...
- **EnclosureLookup port.** *(Enclosure BC)* Cross-BC query port consumed by `start_run` and `start_procedure` pre-flight gates. The gate collects the distinct `located_in_enclosure_id` across the Run or Procedure's Asset scope (widened up the `parent_id` chain so an Asset inherits a functional ancestor's hutch) and loads their permit status via `find_by_ids`. An empty collected-Enclosure set passes the gate trivially (Permit-by-default) and raises neither error. When the set is non-empty, the gate rejects with `RunRequiresPermittedEnclosureError` / `ProcedureRequiresPermittedEnclosureError` when EVERY collected Enclosure fails the Permitted-and-Active check, and `RunEnclosureCoverageMismatchError` / `ProcedureEnclosureCoverageMismatchError` when the set is MIXED (a cross-hutch operation with one hutch Permitted and one not). Ships today with both an `InMemoryEnclosureLookup` (used in tests) and a `PostgresEnclosureLookup` production adapter reading through `proj_enclosure_summary`; the upstream EPICS / P4P / Tango observer adapters that drive the projection are deferred to first pilot integration.
