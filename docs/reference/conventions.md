...
| --- | --- | --- | --- |
| Settings | `family.settings_schema` | `asset.settings` | Slow-moving equipment configuration: pixel offsets, motor calibration values, vendor-specific tuning |
...
**Forward-compatible discipline.** `family.settings_schema` is the strongest candidate to travel between facilities once a publish-pull artifact registry lands. Authoring discipline today avoids a rename pass later. The rules are borrowed from Linux Device Tree bindings: describe the hardware, not the driver; close the schema; reserve namespace expansion slots.

- **Close the schema at every object level.** Set both `additionalProperties: false` and `unevaluatedProperties: false`. Unknown fields are a contract break, not a quiet extension point. A facility that pulls a foreign Family must fail loudly on a property it does not recognize.
...
- **Describe the hardware, not the driver.** Property descriptions name the physical quantity, range, and unit. They do not reference Python classes, EPICS PV names, Tango device URIs, ophyd-async Devices, or any other transport. The same schema must serve an APS-EPICS Family and a MAX-IV-Tango Family without edits.
...
When the slice acts on a per-aggregate SUB-CONCEPT rather than the aggregate itself, the sub-concept noun is the subject: `append_observations` (Run), `append_activities` (Operation), `start_iteration` / `end_iteration` (Operation, the convergence-loop boundary on a Procedure), `revoke_grant` (Trust, a grant is one entry in `Policy.permitted_principal_ids`), `list_at_risk_results` (Agent, a result is one Decision's recorded LLM output, graded for reproducibility by the model catalog). The command class still carries the aggregate qualifier (`AppendProcedureActivities`, `StartProcedureIteration`, `RevokePolicyGrant`) while the slice directory and MCP tool drop it. These sub-concept nouns are tracked in the `_DOMAIN_NOUN_ALLOWLIST` of `tests/architecture/test_slice_verb_names_subject.py`.

The asymmetry is not arbitrary: a slice directory and an MCP tool name are read inside a BC, where the surrounding path supplies the aggregate, but a command class name escapes its namespace. Its `_COMMAND_NAME` is a flat label written into the event envelope's `command_name`, the OTel span name, and the idempotency cache key, where 138 slice labels share one namespace and nothing around the string says which BC produced it. A qualifier the reader can recover from context is redundant; a qualifier the reader cannot recover is load-bearing.

That distinction is also why the logbook-entry family splits the other way. An entry class is a passive row reached only through its full module path (`cora.run.aggregates.run.entries.Observation`), so it takes a bare single-word noun. Its command is not a row, and takes the qualifier. Two deviations predate this clarification and remain open: `AppendInferences` (Decision) and `AppendObservations` (Run) should be `AppendDecisionInferences` and `AppendRunObservations` to match their three Operation siblings.

One allowlisted noun extends the bar beyond persisted sub-concepts: `scan` (Data: `ingest_scan`). A scan is neither an aggregate nor a persisted value type; it is the EXTERNAL artifact (the beamline's scan file) the slice reads before any CORA record of it exists, and no aggregate noun can honestly stand in for it (`ingest_dataset` would name the output as the input). An external-artifact noun earns this only when the slice's whole subject is the artifact, the word has a glossary entry disambiguating its domain overloads, and the code confines it to the ingest seam.

Orchestration slices that drive an aggregate's full lifecycle through a runtime (the Conductor) carry the subject when it reads cleanly (`conduct_procedure`, `conduct_or_hold_procedure`, `conduct_from_procedure` all name `procedure`). The exceptions are the until-loops: `conduct_until_converged`, `conduct_until_advised`, and `conduct_until_advised_from` (all Operation). Each names the loop's TERMINAL CONDITION (conduct the Procedure repeatedly until it converges, or until an adviser says stop) rather than re-stating the Procedure subject, because that predicate is what distinguishes them from `conduct_procedure`. The command classes follow the names and the subject is still the Procedure (driven via start / start_iteration / end_iteration / complete / abort). Each is exempted by its exact fully-qualified name in `_SUBJECT_EXEMPT_SLICES` of `tests/architecture/test_slice_verb_names_subject.py`, never by allowlisting the `converged` or `advised` token, so a slice like `mark_converged` in any BC would still fail the subject guard.
