...
- **Composite affordances.** An Assembly does not declare new affordances that only emerge from the composition; it presents through the Roles it satisfies (`presents_as`), whose required affordances are the contract. Composite-only affordances are reserved for a future revision when a real case appears.
...
Plausible siblings not yet modelled: `TemperatureController` (Lakeshore cryostats), `FlowController` (Bronkhorst mass flow), `PressureController` (MKS Baratron + PID), `DAQController` (Quantum Detectors Merlin, FPGA frame grabbers), `HVPSU` / `BiasController` (CAEN HV crate). The suffix comes last, matching the Family-naming rule above.
...
| `AssetFacilityCodeAssigned` | `asset_id`, `facility_code`, `occurred_at`, `assigned_by` | `bind_asset_to_facility` succeeds; sets the post-genesis Facility binding (set-once; rebind requires decommission + re-register) |
...
`BindAssetToFacility`
: `AssetNotFound`, `AssetFacilityNotFound` (facility_code supplied but the Facility is not visible in the Federation projection), `AssetFacilityCodeAlreadyAssigned` (set-once: the target Asset already carries a facility_code from register_asset genesis OR a prior bind_asset_to_facility call; rebind path is decommission + re-register), `Unauthorized`
