...
- **Composition-root periodic loop.** A background task sweeps on a timer: RunSupervisor watches in-flight Runs and issues `hold_run` / `resume_run` / `stop_run` as facility beam is lost and returns; ClearanceExpirer sweeps Active clearances and issues `expire_clearance` once a validity window has passed; ClearanceWatcher watches front-of-lifecycle clearances (Submitted, UnderReview, Approved) and records a flag Decision when one stalls past an operator window; CalibrationWatcher watches Provisional calibrations and records a flag Decision when one's newest revision has sat unverified past an operator window; ProcedureWatcher watches in-conduct procedures (Running, Held) and records a flag Decision when one has sat past an operator window without progressing, folding in the latest activity recency first so an actively-logging Running conduct is not falsely flagged; CampaignWatcher watches Held campaigns and records a flag Decision when one has sat operator-paused past an operator window without being resumed or closed.
...
| ExperimentCoordinator | deterministic | white-box callable | across-Procedure disposition for a steered experiment (steer again, conclude, or hold the procedure) + an `ExperimentSteering` Decision |
