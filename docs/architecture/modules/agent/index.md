# Agent module <span class="md-maturity md-maturity--stable" title="Eleven slices, eight seeded agents (LLM: RunDebriefer, CautionDrafter; deterministic: RunSupervisor, CautionPromoter, ClearanceExpirer, ClearanceWatcher, CalibrationWatcher, ProcedureWatcher), four-state lifecycle, tool grants, and budget declarations">stable</span>
...
- **Composition-root periodic loop.** A background task sweeps on a timer: RunSupervisor watches in-flight Runs and issues `hold_run` / `resume_run` / `stop_run` as facility beam is lost and returns; ClearanceExpirer sweeps Active clearances and issues `expire_clearance` once a validity window has passed; ClearanceWatcher watches front-of-lifecycle clearances (Submitted, UnderReview, Approved) and records a flag Decision when one stalls past an operator window; CalibrationWatcher watches Provisional calibrations and records a flag Decision when one's newest revision has sat unverified past an operator window; ProcedureWatcher watches in-conduct procedures (Running, Held) and records a flag Decision when one has sat past an operator window without progressing, folding in the latest activity recency first so an actively-logging Running conduct is not falsely flagged.
