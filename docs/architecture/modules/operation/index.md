...
- **Steering within a pass.** A brain advises between passes, so a block's later steps cannot be chosen from its own earlier captures. Mechanically this would be a decide-shaped step over the captures bus that already exists; no pilot workload has asked for it.
...
| `ProcedureIterationEnded` | `procedure_id, iteration_index, converged?, reason?, occurred_at, deciding_brain?` | `end_iteration` accepted; closes the open pass with its convergence verdict. Resets the unconverged streak on `converged=true`, otherwise increments it. A steered pass also records `deciding_brain`, the typed identity of the brain that answered it, naming the deciding leaf rather than a composite. Its `substrate` survives a tier-1 export; the flat `model_ref` beside it is derived from the same value and drops as free text, as do an LLM brain's provider and model. |
...
| `ResolvedStepsRecorded` | `procedure_id, resolved_steps, resolved_closing_steps, step_count, occurred_at` | Pinned at conduct start, before any step executes, after recipe re-expansion and pseudoaxis resolution. Complementary to `RecipeExpansionRecorded`: that one is registration-time and stores hashes for cheap re-derivation, this one is conduct-time and stores the full list for verbatim replay. No-op fold on Procedure state. |
| `SteeringDesignRecorded` | `procedure_id, objective, objective_capture_name, space, budget_iterations_remaining?, budget_wall_clock_seconds_remaining?, substrate, points_per_axis, min_observations, num_restarts, raw_samples, seed, staged_threshold, spend_agent_id?, design_source, occurred_at, brain?` | The design INPUTS of a steered conduct segment, which reach the runtime as command arguments and are persisted nowhere else. A sampling rule is ignorable for inference only if it can be checked, and it can only be checked if it was recorded. An initial conduct writes it in the same append as `ResolvedStepsRecorded`; a resume writes it alone, replaying the pinned steps rather than resolving new ones. Suppressed when the design is identical to the latest already on the stream, so pin count is NOT segment count: attribute a pass to the most recent pin at or before it, and read segment boundaries off `ProcedureHeld` / `ProcedureResumed`. `brain` carries the same determining values typed per substrate, so a segment records only the settings that governed the brain that actually ran, rather than every brain's settings with the irrelevant ones left at their defaults. It is absent on a segment pinned before the field existed, and a design pinned then will re-pin once on its next conduct, because a payload missing a key cannot be compared to one carrying it and the guard fails toward recording. One caveat a reader of the PUBLISHED record needs: the brain settings that survive export are the numeric ones. A language-model brain's provider, model and snapshot pin are free text and are withheld by the same rule that withholds every other unconstrained string here, so its `brain` publishes as empty and is distinguishable from a genuinely settings-free brain only by the `substrate` beside it. No-op fold on Procedure state. |
...
The four examples below follow the canonical Procedure path: register an alignment targeting one Asset, start it, append one setpoint step + one check step, then complete it. The `append_activities` slice carries producer-supplied `event_id` per entry for safe retries (Idempotency-Key is not used at this slice). For the REST/MCP equivalence, auth, and idempotency conventions these examples share, see [Reading the examples](../index.md#reading-the-examples) on the Modules landing page.
