...
| `SteeringDesignRecorded` | `procedure_id, objective, objective_capture_name, space, budget_iterations_remaining?, budget_wall_clock_seconds_remaining?, substrate, points_per_axis, min_observations, num_restarts, raw_samples, seed, staged_threshold, spend_agent_id?, design_source, occurred_at` | The design INPUTS of a steered conduct segment, which reach the runtime as command arguments and are persisted nowhere else. A sampling rule is ignorable for inference only if it can be checked, and it can only be checked if it was recorded. An initial conduct writes it in the same append as `ResolvedStepsRecorded`; a resume writes it alone, replaying the pinned steps rather than resolving new ones. Suppressed when the design is identical to the latest already on the stream, so pin count is NOT segment count: attribute a pass to the most recent pin at or before it, and read segment boundaries off `ProcedureHeld` / `ProcedureResumed`. No-op fold on Procedure state. |
