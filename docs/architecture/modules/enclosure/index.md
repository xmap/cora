...
| `Run` | upstream-of (load-bearing) | `start_run` resolves the Run's Asset scope, widens it UP the `parent_id` chain via `AssetLookup.ancestors_of` (so an Enclosure bound to an ancestor Asset, such as a beamline Unit above a Plan-bound Device, gates the Run; Decommissioned ancestors are dropped from the widening), then reads through the cross-BC `EnclosureLookup.find_for_assets` port. The decider refuses to start the Run when any referencing Enclosure is not Permitted-and-Active. An empty referencing-Enclosure set passes trivially (Permit-by-default). Failure raises `RunRequiresPermittedEnclosureError` (HTTP 409) when EVERY referencing Enclosure fails the gate, and `RunEnclosureCoverageMismatchError` (HTTP 409) when the set is MIXED (at least one passes, at least one fails). |
