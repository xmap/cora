...
|---|---|---|---|
| `Dataset` | `id: UUID` | `id`, `name`, `uri`, `checksum`, `byte_size`, `encoding`, `producing_run_id?`, `producing_procedure_id?`, `subject_id?`, `derived_from: frozenset[UUID]`, `status: DatasetStatus`, `producing_run_end_state: str?`, `producing_actuation_kind: str?`, `intent: Intent`, `used_calibration_ids: frozenset[UUID]` | yes (2-state lifecycle plus orthogonal 3-state Intent) |
...
`Dataset` cross-aggregate references (`producing_run_id`, `producing_procedure_id`, `subject_id`, `derived_from`) are eventual-consistency: the handler pre-loads each referenced aggregate to confirm it exists, the decider applies any further checks (no `derived_from` edges into `Discarded` Datasets), and no fold-time re-validation runs. All are optional: a Dataset can be registered with no producing Run or Procedure (externally-sourced data), no Subject (calibration scans, dark fields), and no upstream lineage (raw data captured at the source). `producing_run_id` and `producing_procedure_id` are the two producer arms: most Datasets come from a Run, while a conducted Procedure that retains data uses `producing_procedure_id` (the secondary arm). `producing_run_end_state` captures the producing Run's terminal status at registration time per the capture-don't-recompute principle, `producing_actuation_kind` snapshots the conduct's observed actuation kind (Physical / Simulated / Hybrid) that powers the promote-gate simulator guard, and `used_calibration_ids` is the AsShot citation set (the `CalibrationRevision.id` values the product actually used), set once and immutable thereafter.
...
|---|---|---|
| `DatasetRegistered` | `dataset_id`, `name`, `uri`, `checksum`, `byte_size`, `encoding`, `producing_run_id?`, `producing_procedure_id?`, `subject_id?`, `derived_from`, `producing_run_end_state?`, `producing_actuation_kind?`, `intent` (always `Trial`), `used_calibration_ids`, `registered_by`, `occurred_at` | `register_dataset` succeeds (genesis) |
