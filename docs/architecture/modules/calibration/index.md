...
- **A closed catalog of quantities.** `CalibrationQuantity` is a closed StrEnum with two shapes. The scalar quantities each carry a single measured number at an operating point: `rotation_center`, `detector_pixel_size`, `magnification`, `effective_thickness`, and `energy_offset` (the monochromator's signed energy correction, true minus commanded, measured by a channel-cut-crystal rocking curve); `blade_throw_scale` is a near-sibling carrying a slit's per-blade pixels-per-mm motion scale (a small fixed set of named-blade scalars rather than a single number, but not a curve), measured by the `blade_throw_characterization` Procedure. The curve-valued (relationship) quantities carry a whole `points` array in one revision: `energy_position_curve` (a positioner's position as a function of beam energy, interpolated between saved points) and its discrete sibling `index_position_table` (a selector's position for each of N named slots, snapped by nearest index). The curve quantities back the `LookupTable` partition rule on energy-driven and discrete-selector [virtual axes](../equipment/index.md#virtual-axes-and-partition-rules). Growth happens by PR, with each quantity declaring its `operating_point_schema` and `value_schema` at import time.
