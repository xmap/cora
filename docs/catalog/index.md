...
The six Roles (`Detector`, `Positioner`, `Controller`, `Sensor`, `Regulator`, `Shutter`) ship as a closed-core seed registry. `Regulator` (drives a continuous process variable to a commanded setpoint; requires the `Settable` affordance) was added when the loose `TemperatureController` family reached the rule-of-three across Diamond i22 / i03 / i11. `Shutter` (gates the beam open or closed on command; requires the `Shutterable` affordance) graduated when three Methods needed to command a shutter as a bound contract: `dark_field`, `flat_field`, and `xpcs`. It is distinct from a `Positioner`, which drives a continuous axis rather than gating, and from a passive BeamStop, which absorbs the beam permanently rather than gating it under command.

A further candidate, `Conditioner` (attenuators and mirrors), stays deferred: no affordance is universally required across those Families, so its required-set would be vacuous, which is the bar `Regulator` cleared because `Settable` is required by every regulator. Shutters are no longer part of that deferred cluster, having earned their own Role on exactly the rule-of-three trigger this paragraph describes.
