...
| TLS / load balancer | nginx vs Caddy vs cloud LB | Deployment chooses its proxy. The live status feed (`docs/stack/deployment.md#live-status-feed`) deliberately did NOT fire this: the relay binds loopback-only and is reached over an SSH tunnel, the same way `/docs` already is, so no proxy sits in front of anything. This row stays open until something actually needs one. |
