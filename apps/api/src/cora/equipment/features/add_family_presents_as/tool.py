"""MCP tool for the `add_family_presents_as` slice."""
...
def register(mcp: FastMCP, *, get_handler: Callable[[], Handler]) -> None:
    """Register the `add_family_presents_as` tool on the given MCP server."""

    @mcp.tool(
        name="add_family_presents_as",
        description=(
            "Add a global Role contract to a Family's presents_as set. "
            "Existence of the Role is verified at the handler edge; the "
            "Family's affordances must superset the Role's "
            "required_affordances or the add is rejected. "
            "Strict-not-idempotent: re-adding a Role already advertised "
            "raises."
        ),
    )
    async def add_family_presents_as_tool(  # pyright: ignore[reportUnusedFunction]
        ...
        handler = get_handler()
        await handler(
            AddFamilyPresentsAs(family_id=family_id, role_id=role_id),
