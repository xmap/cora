...
- 30 items in 2 patterns:
    A. Operational affordances (`-able` / `-ible` / `-ing`): "device
       supports doing X" or "device performs X" — 29 items across
...
Java/Python/Ruby follow informally). All 29 operational affordances
...
class Affordance(StrEnum):
    """Device-level operational primitive. Closed v1 enum (30 items, 2 patterns).
    ...
    Pattern A, Operational affordances (`-able` / `-ible` / `-ing`):
        ...
        (role/flow). Set-membership claim. 29 items.
    ...
    SETTABLE = "Settable"
    """Accepts a commanded continuous setpoint and drives a process variable toward
    it (temperature, flow, pressure); mechanism-agnostic (PID loop or otherwise)."""
...
class InvalidAffordanceError(ValueError):
    ...
    def __init__(self, value: object) -> None:
        super().__init__(
            f"Invalid Affordance value: {value!r}; must be one of the 30 "
