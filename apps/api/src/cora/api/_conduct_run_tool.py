...
def register(mcp: FastMCP, *, get_runtime: Callable[[], ComputeRuntime]) -> None:
    """Register the `conduct_run` tool on the given MCP server."""

    @mcp.tool(
        name="conduct_run",
        description=(
            "Conduct a compute Run end-to-end: submit the supplied job "
            "(command argv + input/output URIs) to the ComputePort, await "
            "its terminal state, and complete the Run on success or abort "
            "it on failure. The conduct-observed actuation kind is captured "
            "on the Run for provenance. Returns a structured summary; "
...
def register_conduct_run_tools(mcp: FastMCP, *, get_runtime: Callable[[], ComputeRuntime]) -> None:
    """Register all compute MCP tools (currently just conduct_run)."""
    register(mcp, get_runtime=get_runtime)
