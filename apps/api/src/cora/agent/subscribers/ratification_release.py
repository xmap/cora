...
CORRELATION (the load-bearing guard, `last_hold_placed_by_enforcer`): the release
resumes ONLY a hold this enforcer placed. RunStatus.HELD carries no hold-source, so
a run may be Held by the kill-switch, an operator, or this gate; resuming blindly
on status == HELD would let a stray Granted/Denied defeat a foreign hold. The
guard folds the run's most-recent RunHeld and resumes only when its envelope
principal_id is this enforcer; otherwise it stands down.
...
from cora.run.aggregates.run import RunResumed, RunStatus
...
class RatificationReleaseSubscriber:
    ...
    async def _resume(self, run_id: UUID) -> None:
        ...
        if state is None or state.status is not RunStatus.HELD:
            ...
            return
        if not last_hold_placed_by_enforcer(stored):
            # The run is Held for a DIFFERENT reason (kill-switch / operator). Do
            # not resume it: clearing someone else's hold would invert their intent
            # (e.g. defeat the kill-switch). Stand down; the other holder owns it.
        ...
        if isinstance(authz, Deny):
            ...
            return
        now = self.clock.now()
        resumed = RunResumed(run_id=run_id, occurred_at=now, decided_by_decision_id=None)
        envelope = to_new_event(
            event_type=run_event_type_name(resumed),
            payload=run_to_payload(resumed),
            occurred_at=now,
