"""Bootstrap-time seed for the ExperimentCoordinator Agent.

The ExperimentCoordinator is CORA's L3 coordination agent for autonomous
...
iteration's advice on `ProcedureIterationEnded`; the ExperimentCoordinator's distinct
...
signed agent-write path (`cora.api._experiment_coordinator`).

This seed gives the agent an Agent record (and its co-registered Actor) at the
pinned `EXPERIMENT_COORDINATOR_AGENT_ID` so it can author Decisions
(`decided_by = ActorId(EXPERIMENT_COORDINATOR_AGENT_ID)`) and issue follow-on
...
`cora.agent._agent_seed`.
  ...
  - Pinned UUID in the `5733XXXX` range (a fresh deployment-stable range,
    ...
    ExperimentCoordinator-authored Decision.
  - DETERMINISTIC agent (the steering brain is the DecidePort, NOT an LLM): no
    ...
    (`BrainRef.for_rule("ExperimentCoordinator:v1")`). Same posture as RunSupervisor
    / RunInitiator.
...
# ---------------------------------------------------------------------------
# ExperimentCoordinator agent identity (deployment-stable constants)
# ---------------------------------------------------------------------------

# Treat as FOREVER-STABLE. Same change-cost rationale as the sibling agents:
# changing this orphans every prior ExperimentCoordinator-authored Decision (their
...
# grouped per agent.
EXPERIMENT_COORDINATOR_AGENT_ID = UUID("01900000-0000-7000-8000-000057330010")
EXPERIMENT_COORDINATOR_AGENT_NAME = "ExperimentCoordinator"
EXPERIMENT_COORDINATOR_AGENT_KIND = "ExperimentCoordinator"
EXPERIMENT_COORDINATOR_AGENT_VERSION = "1.0.0"
EXPERIMENT_COORDINATOR_AGENT_DESCRIPTION = (
...
async def seed_experiment_coordinator_agent(kernel: Kernel) -> None:
    """Seed the ExperimentCoordinator Agent + co-registered Actor (idempotent)."""
    identity = AgentSeedIdentity(
        agent_id=EXPERIMENT_COORDINATOR_AGENT_ID,
        name=EXPERIMENT_COORDINATOR_AGENT_NAME,
        kind=EXPERIMENT_COORDINATOR_AGENT_KIND,
        version=EXPERIMENT_COORDINATOR_AGENT_VERSION,
        description=EXPERIMENT_COORDINATOR_AGENT_DESCRIPTION,
        ...
        command_name="SeedExperimentCoordinatorAgent",
    )
