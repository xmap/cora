    ...
    match event:
        ...
        case ProcedureStarted():
            ...
            return Procedure(
                ...
                diagnostic_logbook_id=prior.diagnostic_logbook_id,
        ...
        case ProcedureDiagnosticLogbookOpened(logbook_id=logbook_id):
            # Lazy open-on-first-write for the GP-steering diagnostics logbook:
            # preserve all prior state, set diagnostic_logbook_id. Status NOT
            # touched -- orthogonal to lifecycle, exactly like the activities
            # logbook arm above.
            prior = require_state(state, "ProcedureDiagnosticLogbookOpened")
            return Procedure(
                id=prior.id,
                name=prior.name,
                kind=prior.kind,
                ...
                parent_run_id=prior.parent_run_id,
                activity_logbook_id=prior.activity_logbook_id,
                diagnostic_logbook_id=logbook_id,
                outcome_logbook_id=prior.outcome_logbook_id,
                capability_id=prior.capability_id,
                recipe_id=prior.recipe_id,
                current_iteration_index=prior.current_iteration_index,
        ...
        case ProcedureOutcomeLogbookOpened(logbook_id=logbook_id):
            # Lazy open-on-first-write for the steered-pass outcome logbook:
            # preserve all prior state, set outcome_logbook_id. Status NOT
            # touched -- orthogonal to lifecycle, exactly like the activities +
            # diagnostic logbook arms above.
            prior = require_state(state, "ProcedureOutcomeLogbookOpened")
            return Procedure(
                id=prior.id,
                name=prior.name,
                kind=prior.kind,
