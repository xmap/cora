...
class _ToolResult(BaseModel):
    ...
    substrate_writes: dict[str, int | float | bool | str | list[Any]] = Field(
        ...
        description=(
            ...
            "carrying the last value written to each. Present on a HELD "
            "outcome too: a paused Procedure's closing steps have not run."
        ),
    )
    closing_failures: list[dict[str, Any]] = Field(
        default_factory=list[dict[str, Any]],
        description=(
            "Every closing step that failed. Always empty on `held=True`. Never flips `succeeded`."
        ),
    )
