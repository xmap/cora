...
class RedactionResult:
    ...
    `unfired_tier1_fields` is the same completeness fact one tier up:
    every `(event_type, field)` pair DECLARED in `DISPOSITIONS` for an
    event type THIS export carried, whose field never actually appeared
    on any row of that type. Unlike tier 2's hand-curated allowlist,
    tier 1 is exhaustively generated by Step 0's generator, so most of
    the time every declared field fires (a payload key is present, even
    as `null`, on every schema version that still declares it); the case
    this catches is an older `schema_version` row whose payload predates
    a field the CURRENT dataclass declares. This is a narrowness caveat,
    not a leak: a field that never fired was never published either way.
    A rule that is dead in EVERY export (one whose declared key can never
    match anything the store actually writes, as opposed to one export's
    narrow coverage) is a different problem -- a mismatch between what
    the generator resolved from the dataclass and what `to_payload`
    actually serializes -- and needs a build-time check comparing the two
    directly; this per-export field cannot see that case and does not
    claim to.
    """
    ...
    unfired_tier1_fields: frozenset[tuple[str, str]]
...
def redact_record(
    ...
    unfired_tier2 = unfired_clearances(fired_pointers, kinds_present=frozenset(record.logbooks))

    event_types_present = {str(row["event_type"]) for row in record.streams}
    fired_fields = tier1.fired_fields
    unfired_tier1 = frozenset(
        (event_type, field)
        for event_type in event_types_present
        # `tier1.redact_row` above already raised UnknownEventTypeError for
        # any event_type not in DISPOSITIONS, so every member of
        # event_types_present is guaranteed present here.
        for field in DISPOSITIONS[event_type]
        if field not in fired_fields.get(event_type, frozenset())
    )

    return RedactionResult(
        redacted_record=RedactedRecord(streams=redacted_streams, logbooks=redacted_logbooks),
        token_map=token_map,
        unfired_tier2_clearances=unfired_tier2,
