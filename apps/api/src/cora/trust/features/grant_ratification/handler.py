"""Application handler for the `grant_ratification` slice.

Transition longhand handler: loads current Ratification state, threads the
envelope `principal_id` into the decider as `granted_by` (the independence check
...
_COMMAND_NAME = "GrantRatification"

_log = get_logger(__name__)


class Handler(Protocol):
    """Bare grant_ratification handler -- what `bind()` returns."""
...
def bind(deps: Kernel) -> Handler:
    """Build a grant_ratification handler closed over the shared deps."""
