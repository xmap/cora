"""Stub route module for `record_watched_run` (in-process-only slice).

Per the roadmap's anti-scope: no operator path to a watched genesis. No
...
that stops the watched path being used to launder around a driven
refusal. In-process adapters (the capture-watch runtime) call
`RunHandlers.record_watched_run(...)` directly.
