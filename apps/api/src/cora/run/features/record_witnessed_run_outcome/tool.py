"""Stub MCP tool module for `record_witnessed_run_outcome` (in-process-only slice).

Per the roadmap's anti-scope: this slice is NOT exposed as an MCP tool.
In-process adapters call `RunHandlers.record_witnessed_run_outcome(...)`
directly.
...
from cora.run.features.record_witnessed_run_outcome.handler import Handler

_STUB_RESOLVER = get_mcp_surface_id


def register(mcp: FastMCP, *, get_handler: Callable[[], Handler]) -> None:
    """No-op MCP registration: record_witnessed_run_outcome is in-process-only."""
    _ = mcp
    _ = get_handler
    _ = _STUB_RESOLVER
