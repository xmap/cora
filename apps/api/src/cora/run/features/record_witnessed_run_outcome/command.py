...
opened. Carries only the substrate-observed facts:
  ...
  - `run_id` -- the Run to terminate. Resolved by the RunTranslator runtime
  ...
  - `monitor_source_id` -- the stable `MonitorSourceId` of the in-process
    RunTranslator runtime, mirroring `RecordWitnessedRun.monitor_source_id`.
  ...
  - `capture_progress_snapshot` -- the last per-role progress counts
    RunTranslator retained before this terminal, or `None` if nothing was
...
`decided_by_decision_id`: RunTranslator has no Decision-BC input to link.
