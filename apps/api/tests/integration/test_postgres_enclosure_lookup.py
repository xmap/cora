...
semantics are pinned via an unseeded id, and the `find_by_ids`
filter is pinned against a set of enclosure ids plus the
...
async def _seed_enclosure(db_pool: asyncpg.Pool, *, name: str, facility_code: str = "cora") -> UUID:
    # build_postgres_deps seeds an Active "cora" Facility in its default
    # in-memory FacilityLookup, so the default facility_code resolves.
...
async def test_lookup_returns_populated_reference_for_registered_enclosure(
    ...
    enclosure_id = await _seed_enclosure(db_pool, name=name)
...
async def test_lookup_returns_permitted_status_after_observation(db_pool: asyncpg.Pool) -> None:
    suffix = uuid4().hex[:8]
    enclosure_id = await _seed_enclosure(db_pool, name=f"hutch-B-{suffix}")
    await _observe(
...
async def test_lookup_isolates_records_by_id(db_pool: asyncpg.Pool) -> None:
    ...
    enc_a = await _seed_enclosure(db_pool, name=f"hutch-A-{suffix_a}")
    enc_b = await _seed_enclosure(db_pool, name=f"hutch-B-{suffix_b}")
...
async def test_lookup_returns_decommissioned_lifecycle(db_pool: asyncpg.Pool) -> None:
    ...
    `find_by_ids` excludes Decommissioned, but `lookup` does not."""
    suffix = uuid4().hex[:8]
    enc = await _seed_enclosure(db_pool, name=f"hutch-tomb-{suffix}")
...
@pytest.mark.integration
async def test_find_by_ids_returns_enclosures_for_matching_ids(db_pool: asyncpg.Pool) -> None:
    suffix = uuid4().hex[:8]
    enc_a = await _seed_enclosure(db_pool, name=f"hutch-target-A-{suffix}")
    enc_b = await _seed_enclosure(db_pool, name=f"hutch-target-B-{suffix}")
    enc_other = await _seed_enclosure(db_pool, name=f"hutch-other-{suffix}")
    await _drain_enclosure(db_pool)

    lookup = PostgresEnclosureLookup(db_pool)
    results = await lookup.find_by_ids(enclosure_ids=frozenset({enc_a, enc_b}))
...
@pytest.mark.integration
async def test_find_by_ids_empty_input_returns_empty(db_pool: asyncpg.Pool) -> None:
    lookup = PostgresEnclosureLookup(db_pool)
    assert await lookup.find_by_ids(enclosure_ids=frozenset()) == []


@pytest.mark.integration
async def test_find_by_ids_excludes_decommissioned(db_pool: asyncpg.Pool) -> None:
    """Decommissioned enclosures must not gate runs; the
    `lifecycle = 'Active'` partial-index posture is respected end-to-end."""
    suffix = uuid4().hex[:8]
    enc_active = await _seed_enclosure(db_pool, name=f"hutch-active-{suffix}")
    enc_tomb = await _seed_enclosure(db_pool, name=f"hutch-tomb-{suffix}")
    await _decommission(db_pool, enclosure_id=enc_tomb, now=_T2)
    await _drain_enclosure(db_pool)

    lookup = PostgresEnclosureLookup(db_pool)
    results = await lookup.find_by_ids(enclosure_ids=frozenset({enc_active, enc_tomb}))
    returned = {r.enclosure_id for r in results}
