"""RunSupervisor autonomously aborts a low-quality Run at APS 2-BM (Rule Q act rung).
...
End-to-end proof of the quality act rung: a quality channel reads below the
operator limit for the settle window, so the supervisor issues AbortRun (the
emergency-exit terminal that flags the data suspect), linking an Abort Decision.

Everything is real Postgres: the Run is walked to Running through the real
start_run + projections, the per-Run snr_limit flows from
RunStarted.effective_parameters through the real RunSummaryProjection + list_runs
drain, and the abort is the real bind_abort_run handler writing a real RunAborted
+ Decision stream. The one injected fake is the observation channel-feed (a
hand-seeded InMemoryRunChannelLookup, the same seam the auto-resume scenario uses
for beam availability): the supervisor tick runs on a FakeClock, while the
Postgres observation table stamps recorded_at via DEFAULT now() (real wall
clock), so a frozen-clock test controls channel freshness through the in-memory
lookup. The PostgresRunChannelLookup read path itself is covered by
test_run_channel_lookup_postgres.py.
"""
...
_CORRELATION_ID = UUID("01900000-0000-7000-8000-0000004661bb")

_QUALITY_CHANNEL = "snr"
_SNR_LIMIT = 5.0

# Scenario tag: 466 (RunSupervisor quality -> abort).
_APS_SITE_ID = UUID("01900000-0000-7000-8000-000000466501")
_2BM_UNIT_ID = UUID("01900000-0000-7000-8000-000000466a01")
...
_ASSET_AEROTECH_ABRS_ID = UUID("01900000-0000-7000-8000-000000466a11")
_ASSET_SAMPLE_TOP_X_ID = UUID("01900000-0000-7000-8000-000000466a21")
_ASSET_ORYX_5MP_ID = UUID("01900000-0000-7000-8000-000000466a31")
_ASSET_SCINTILLATOR_LUAG_ID = UUID("01900000-0000-7000-8000-000000466a41")

_PI_ACTOR_ID = UUID("01900000-0000-7000-8000-000000466b01")
_SUBJECT_ID = UUID("01900000-0000-7000-8000-000000466b11")
_CAMPAIGN_ID = UUID("01900000-0000-7000-8000-000000466b21")
_CLEARANCE_ID = UUID("01900000-0000-7000-8000-000000466f01")
_ESAF_TEMPLATE_ID = ClearanceTemplateId(clearance_template_stream_id("cora", "ESAF"))
_METHOD_ID = UUID("01900000-0000-7000-8000-000000466d01")
_CAPABILITY_ID = UUID("01900000-0000-7000-8000-000000c0d466")
_PRACTICE_ID = UUID("01900000-0000-7000-8000-000000466d11")
_PLAN_ID = UUID("01900000-0000-7000-8000-000000466d21")
_RUN_ID = UUID("01900000-0000-7000-8000-000000466f02")
...
_BEAMTIME = BeamtimeSpec(
    ...
    pi_actor_name="Proposal 2026-4661 PI",
    subject_id=_SUBJECT_ID,
    subject_name="porous sandstone core (Proposal 2026-4661, sample A, quality-abort)",
    campaign_id=_CAMPAIGN_ID,
    campaign_name="Proposal 2026-4661 beamtime (quality-abort)",
...
_RECIPE = RecipeSpec(
    ...
    parameters_schema={
        ...
        "properties": {
            ...
            "snr_limit": {"type": "number", "minimum": 0},
        },
...
_RULES_QUALITY = ObservationRuleConfig(
    quality_channel_name=_QUALITY_CHANNEL,
...
def _id_queue() -> list[UUID]:
    ...
    (drain correlations, the Decision id, the AbortRun command correlation)."""
    e = uuid4
    return [
...
async def _setup_running_run(deps: Kernel, db_pool: asyncpg.Pool) -> None:
    ...
    snr_limit, gated by a real Active ESAF Clearance bound to the Subject."""
    ...
    await bind_mount_subject(deps)(
        MountSubject(
            ...
            reason="quality-abort scenario setup",
        ),
        principal_id=_PRINCIPAL_ID,
        correlation_id=_CORRELATION_ID,
    )
    ...
    await bind_register_clearance(deps)(
        RegisterClearance(
            template_id=_ESAF_TEMPLATE_ID,
            facility_code="cora",
            title="Proposal 2026-4661 ESAF (porous sandstone tomography)",
    ...
    await bind_start_run(deps)(
        StartRun(
            name="Proposal 2026-4661 sample A tomography (quality-abort)",
            plan_id=_PLAN_ID,
            subject_id=_SUBJECT_ID,
            override_parameters={
                "exposure_ms": 100,
                ...
                "snr_limit": _SNR_LIMIT,
            },
            trigger_source="operator-manual; supervisor quality-abort scenario",
        ),
        principal_id=_PRINCIPAL_ID,
        correlation_id=_CORRELATION_ID,
    )
...
async def _tick(
    ...
    *,
    ...
    quality act rung enabled; all non-quality rungs off."""
...
async def test_supervisor_aborts_run_after_quality_below_limit_settles(
    db_pool: asyncpg.Pool,
) -> None:
    """A quality channel below the limit for the settle window -> the supervisor
    aborts the Run, linking an Abort Decision; the Run reaches Aborted."""
    ...
    lookup.register(run_id=_RUN_ID, channel_name=_QUALITY_CHANNEL, value=3.0, recorded_at=_NOW)
    quality: set[UUID] = set()
    quality_act_settle: dict[UUID, int] = {}

    # Tick 1: below limit, settle window (2) not met -> no abort yet.
    await _tick(deps, lookup, quality_act_settle=quality_act_settle, quality=quality)
    events, _ = await deps.event_store.load("Run", _RUN_ID)
    assert [e.event_type for e in events] == ["RunStarted"]
    assert quality_act_settle[_RUN_ID] == 1

    # Tick 2: still below, settle window met -> abort fires.
    await _tick(deps, lookup, quality_act_settle=quality_act_settle, quality=quality)
    events, _ = await deps.event_store.load("Run", _RUN_ID)
    assert [e.event_type for e in events] == ["RunStarted", "RunAborted"]
    aborted = next(e for e in events if e.event_type == "RunAborted")
    decision_id = aborted.payload["decided_by_decision_id"]
...
@pytest.mark.integration
async def test_supervisor_does_not_abort_when_quality_recovers_before_settle(
    db_pool: asyncpg.Pool,
) -> None:
    """A transient dip that recovers within limits before the settle window never
    aborts: the Run stays Running."""
    deps = build_postgres_deps(
        db_pool,
        now=_NOW,
        ids=_id_queue(),
    ...
    # Tick 1: below limit (counter -> 1).
    lookup.register(run_id=_RUN_ID, channel_name=_QUALITY_CHANNEL, value=3.0, recorded_at=_NOW)
    await _tick(deps, lookup, quality_act_settle=quality_act_settle, quality=quality)
    assert quality_act_settle[_RUN_ID] == 1

    # Tick 2: recovered within limits (later recorded_at wins) -> counter resets, no abort.
    lookup.register(
        run_id=_RUN_ID,
        channel_name=_QUALITY_CHANNEL,
        value=9.0,
        recorded_at=datetime(2026, 5, 18, 2, 0, 1, tzinfo=UTC),
    )
    await _tick(deps, lookup, quality_act_settle=quality_act_settle, quality=quality)

    events, _ = await deps.event_store.load("Run", _RUN_ID)
    assert [e.event_type for e in events] == ["RunStarted"]
    assert _RUN_ID not in quality_act_settle
