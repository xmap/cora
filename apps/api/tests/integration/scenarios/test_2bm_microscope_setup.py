...
async def test_microscope_deployment_plays_out_end_to_end(db_pool: asyncpg.Pool) -> None:
    ...
    # ----- NEW Families (Imager presenter, Objective, PseudoAxis,
    #       Housing) + schemas. The turret is a LinearStage (sliding
    #       ball-screw selector), so LinearStage (registered above) covers
    #       it and no RotaryStage Family is needed in this deployment. -----
    for name in ("Imager", "Objective", "PseudoAxis", "Housing"):
