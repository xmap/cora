"""Contract tests for the `append_diagnostics` MCP tool."""
...
def test_mcp_lists_append_diagnostics_tool() -> None:
    ...
    assert "append_diagnostics" in tool_names


@pytest.mark.contract
def test_mcp_append_diagnostics_tool_succeeds_for_running() -> None:
    with TestClient(create_app()) as client:
        headers = open_session(client)
