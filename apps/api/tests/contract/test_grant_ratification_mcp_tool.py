"""Contract tests for the `grant_ratification` MCP tool.
...
REST with a distinct `X-Principal-Id`, so the MCP grant is by a different
...
def test_mcp_lists_grant_ratification_tool() -> None:
    ...
    assert "grant_ratification" in tool_names


@pytest.mark.contract
def test_mcp_grant_ratification_tool_returns_structured_ratification_id() -> None:
    with TestClient(create_app()) as client:
...
@pytest.mark.contract
def test_mcp_grant_ratification_tool_returns_iserror_on_unknown_id() -> None:
...
@pytest.mark.contract
def test_mcp_grant_ratification_tool_rejects_missing_argument() -> None:
    with TestClient(create_app()) as client:
        session_headers = open_session(client)
        response = client.post(
            "/mcp",
