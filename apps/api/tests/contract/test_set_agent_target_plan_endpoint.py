...
def test_post_set_target_plan_returns_204() -> None:
...
@pytest.mark.contract
def test_post_set_target_plan_clears_with_null() -> None:
    with TestClient(create_app()) as client:
        define = client.post("/agents", json=_define_body())
        agent_id = define.json()["agent_id"]
        client.post(
...
@pytest.mark.contract
def test_post_set_target_plan_clears_with_empty_body() -> None:
    with TestClient(create_app()) as client:
        define = client.post("/agents", json=_define_body())
        agent_id = define.json()["agent_id"]
...
@pytest.mark.contract
def test_post_set_target_plan_404_on_unknown_id() -> None:
    with TestClient(create_app()) as client:
        response = client.post(
...
@pytest.mark.contract
def test_post_set_target_plan_409_on_deprecated_agent() -> None:
    with TestClient(create_app()) as client:
        define = client.post("/agents", json=_define_body())
        agent_id = define.json()["agent_id"]
...
@pytest.mark.contract
def test_post_set_target_plan_422_on_malformed_uuid() -> None:
    with TestClient(create_app()) as client:
        define = client.post("/agents", json=_define_body())
        agent_id = define.json()["agent_id"]
        response = client.post(
...
@pytest.mark.contract
def test_post_set_target_plan_idempotent_returns_204_on_same_plan() -> None:
    plan_id = str(uuid4())
    with TestClient(create_app()) as client:
        define = client.post("/agents", json=_define_body())
        agent_id = define.json()["agent_id"]
