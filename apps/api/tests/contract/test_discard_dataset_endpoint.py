...
def test_post_discard_dataset_round_trips_into_get_dataset_status_discarded() -> None:
    with TestClient(create_app()) as client:
        dataset_id = _register(client)
        client.post(f"/datasets/{dataset_id}/discard", json={"reason": "Superseded"})
...
def test_post_discard_dataset_returns_409_when_already_discarded() -> None:
    ...
    with TestClient(create_app()) as client:
        ...
        second = client.post(f"/datasets/{dataset_id}/discard", json={"reason": "Superseded"})
...
def test_post_discard_dataset_rejects_empty_reason_with_422() -> None:
    with TestClient(create_app()) as client:
        dataset_id = _register(client)
        response = client.post(f"/datasets/{dataset_id}/discard", json={"reason": "Superseded"})
...
def test_post_datasets_409_message_lists_all_discarded_derived_from_ids() -> None:
    with TestClient(create_app()) as client:
        ...
        client.post(f"/datasets/{a}/discard", json={"reason": "Superseded"})
        client.post(f"/datasets/{b}/discard", json={"reason": "Superseded"})
        response = client.post(
            "/datasets",
            json={
                "name": "derived",
