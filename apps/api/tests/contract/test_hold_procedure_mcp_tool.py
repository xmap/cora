"""Contract tests for the `hold_procedure` MCP tool."""
...
def test_mcp_lists_hold_procedure_tool() -> None:
    ...
    assert "hold_procedure" in tool_names


@pytest.mark.contract
def test_mcp_hold_procedure_tool_succeeds_for_running() -> None:
    with TestClient(create_app()) as client:
        headers = open_session(client)
