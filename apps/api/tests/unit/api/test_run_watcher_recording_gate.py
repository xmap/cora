...
"""Unit tests for the RunWatcher recording boot guard.

`_enforce_run_watcher_recording_gate` refuses to boot with
`run_watcher_recording_enabled=True` unless both `run_watcher_enabled`
...
from cora.api.main import _enforce_run_watcher_recording_gate
...
def _settings(
    *,
    ...
    return Settings(  # type: ignore[call-arg]
        run_watcher_enabled=run_watcher_enabled,
...
def test_recording_disabled_is_always_a_no_op() -> None:
    for run_watcher_enabled in (True, False):
        for capture_watch_plan_id in (None, uuid4()):
            _enforce_run_watcher_recording_gate(
                _settings(
                    run_watcher_enabled=run_watcher_enabled,
                    capture_watch_plan_id=capture_watch_plan_id,
                    run_watcher_recording_enabled=False,
                )
            )


def test_recording_enabled_without_watcher_enabled_refuses_boot() -> None:
    with pytest.raises(RuntimeError, match="RUN_WATCHER_ENABLED=true"):
        _enforce_run_watcher_recording_gate(
            _settings(
                run_watcher_enabled=False,
                capture_watch_plan_id=uuid4(),
                run_watcher_recording_enabled=True,
            )
        )
...
def test_recording_enabled_missing_both_refuses_boot_naming_both_vars() -> None:
    ...
    assert "RUN_WATCHER_ENABLED=true" in message
