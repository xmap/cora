"""Property-based tests for `degrade_supply.decide` (Supply BC).

Complements the example-based `test_degrade_supply_decider.py` with
universal claims across generated inputs. The decider is a pure
operator-driven FSM transition

    (state, command, now, triggered_by) -> list[SupplyDegraded]
...
and field threading:
  ...
  - Every source status in the permitted set `{Unknown, Available,
    Recovering}` emits exactly one `SupplyDegraded` whose audit fields
    (supply_id, from_status, reason, trigger, triggered_by,
    occurred_at) are the threaded inputs; trigger is always "Operator"
    and monitor_ref is absent (operator path).
  - Every source status outside the permitted set (Degraded,
    Unavailable, Decommissioned) always raises `SupplyCannotDegradeError`
    carrying the current status. The partition is total over
    `SupplyStatus`, so a future status value cannot silently fall
    through.
...
def _command(*, supply_id: UUID, reason: str) -> DegradeSupply:
    return DegradeSupply(supply_id=supply_id, reason=reason)
...
def test_degrade_from_permitted_source_emits_single_event(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any permitted source emits one SupplyDegraded with threaded fields."""
    triggered_by = ActorId(triggered_by_uuid)
...
def test_degrade_from_disallowed_source_always_raises_cannot_degrade(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any source outside {Unknown, Available, Recovering} raises, carrying status."""
    with pytest.raises(SupplyCannotDegradeError) as exc:
        degrade_supply.decide(
            state=_state(supply_id=supply_id, status=source),
            command=_command(supply_id=supply_id, reason=reason),
            now=now,
...
def test_degrade_uses_state_id_not_command_supply_id(
...
def test_degrade_is_pure_same_input_same_output(
    ...
    command = _command(supply_id=supply_id, reason=reason)
    triggered_by = ActorId(triggered_by_uuid)
    first = degrade_supply.decide(state=state, command=command, now=now, triggered_by=triggered_by)
    second = degrade_supply.decide(state=state, command=command, now=now, triggered_by=triggered_by)
    assert first == second
