"""Property-based tests for `restore_supply.decide` (Supply BC).

Complements the example-based `test_restore_supply_decider.py` with
universal claims across generated inputs. The decider is a pure
operator-driven FSM transition

    (state, command, now, triggered_by) -> list[SupplyRestored]
...
and field threading:
  ...
  - state=None always raises `SupplyNotFoundError`, regardless of
    command shape.
  - Every source status in the permitted set `{Recovering}` (Recovering
    only, single-source recovery acknowledgement) emits exactly one
    `SupplyRestored` whose audit fields (supply_id, from_status, reason,
    trigger, triggered_by, occurred_at) are the threaded inputs; trigger
    is always "Operator" and monitor_ref is absent (operator path).
  - Every source status outside the permitted set always raises
    `SupplyCannotRestoreError` carrying the current status. The partition
    is total over `SupplyStatus`, so a future status value cannot
...
def _command(*, supply_id: UUID, reason: str) -> RestoreSupply:
    return RestoreSupply(supply_id=supply_id, reason=reason)
...
def test_restore_from_permitted_source_emits_single_event(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any permitted source emits one SupplyRestored with threaded fields."""
    triggered_by = ActorId(triggered_by_uuid)
    ...
    assert len(events) == 1
    event = events[0]
    assert isinstance(event, SupplyRestored)
...
def test_restore_from_disallowed_source_always_raises_cannot_restore(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any source outside {Recovering} raises, carrying status."""
    with pytest.raises(SupplyCannotRestoreError) as exc:
        restore_supply.decide(
            state=_state(supply_id=supply_id, status=source),
            command=_command(supply_id=supply_id, reason=reason),
            now=now,
...
def test_restore_uses_state_id_not_command_supply_id(
...
def test_restore_is_pure_same_input_same_output(
    ...
    command = _command(supply_id=supply_id, reason=reason)
    triggered_by = ActorId(triggered_by_uuid)
    first = restore_supply.decide(state=state, command=command, now=now, triggered_by=triggered_by)
    second = restore_supply.decide(state=state, command=command, now=now, triggered_by=triggered_by)
    assert first == second
