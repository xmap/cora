"""Property-based tests for `mark_supply_available.decide` (Supply BC).

Complements the example-based `test_mark_supply_available_decider.py` with
universal claims across generated inputs. The decider is a pure
operator-driven FSM transition

    (state, command, now, triggered_by) -> list[SupplyMarkedAvailable]
...
and field threading:
  ...
  - state=None always raises `SupplyNotFoundError`, regardless of
    command shape.
  - Every source status in the permitted set `{Unknown}` (Unknown
    only, single-source first-observation declaration) emits exactly
    one `SupplyMarkedAvailable` whose audit fields (supply_id,
  ...
  - Every source status outside the permitted set (Available,
    Degraded, Unavailable, Recovering, Decommissioned) always raises
    `SupplyCannotMarkAvailableError` carrying the current status. The
...
_PERMITTED_SOURCES = (SupplyStatus.UNKNOWN,)
...
def _command(*, supply_id: UUID, reason: str) -> MarkSupplyAvailable:
    return MarkSupplyAvailable(supply_id=supply_id, reason=reason)
...
def test_mark_available_with_none_state_always_raises_not_found(
    supply_id: UUID,
    reason: str,
    now: datetime,
...
def test_mark_available_from_permitted_source_emits_single_event(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any permitted source emits one SupplyMarkedAvailable with threaded fields."""
    triggered_by = ActorId(triggered_by_uuid)
    ...
    assert len(events) == 1
    event = events[0]
    assert isinstance(event, SupplyMarkedAvailable)
...
def test_mark_available_from_disallowed_source_always_raises_cannot_mark_available(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any source outside {Unknown} raises, carrying status."""
    with pytest.raises(SupplyCannotMarkAvailableError) as exc:
        mark_supply_available.decide(
            state=_state(supply_id=supply_id, status=source),
            command=_command(supply_id=supply_id, reason=reason),
            now=now,
...
def test_mark_available_uses_state_id_not_command_supply_id(
...
def test_mark_available_is_pure_same_input_same_output(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    command = _command(supply_id=supply_id, reason=reason)
    triggered_by = ActorId(triggered_by_uuid)
    first = mark_supply_available.decide(
        state=state, command=command, now=now, triggered_by=triggered_by
    )
    second = mark_supply_available.decide(
        state=state, command=command, now=now, triggered_by=triggered_by
    )
    assert first == second
