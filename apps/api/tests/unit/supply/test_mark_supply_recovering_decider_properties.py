"""Property-based tests for `mark_supply_recovering.decide` (Supply BC).

Complements the example-based `test_mark_supply_recovering_decider.py` with
universal claims across generated inputs. The decider is a pure
operator-driven FSM transition

    (state, command, now, triggered_by) -> list[SupplyMarkedRecovering]
...
and field threading:
  ...
  - state=None always raises `SupplyNotFoundError`, regardless of
    command shape.
  - Every source status in the permitted set `{Unavailable}` (Unavailable
    only, single-source) emits exactly one `SupplyMarkedRecovering` whose
    audit fields (supply_id, from_status, reason, trigger, triggered_by,
    occurred_at) are the threaded inputs; trigger is always "Operator"
    and monitor_ref is absent (operator path).
  - Every source status outside the permitted set always raises
    `SupplyCannotMarkRecoveringError` carrying the current status. The
...
_PERMITTED_SOURCES = (SupplyStatus.UNAVAILABLE,)
...
def _command(*, supply_id: UUID, reason: str) -> MarkSupplyRecovering:
    return MarkSupplyRecovering(supply_id=supply_id, reason=reason)
...
def test_mark_recovering_with_none_state_always_raises_not_found(
    supply_id: UUID,
    reason: str,
    now: datetime,
...
def test_mark_recovering_from_permitted_source_emits_single_event(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any permitted source emits one SupplyMarkedRecovering with threaded fields."""
    triggered_by = ActorId(triggered_by_uuid)
...
def test_mark_recovering_from_disallowed_source_always_raises_cannot_mark_recovering(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    """Any source outside {Unavailable} raises, carrying status."""
    with pytest.raises(SupplyCannotMarkRecoveringError) as exc:
        mark_supply_recovering.decide(
            state=_state(supply_id=supply_id, status=source),
            command=_command(supply_id=supply_id, reason=reason),
            now=now,
...
def test_mark_recovering_uses_state_id_not_command_supply_id(
...
def test_mark_recovering_is_pure_same_input_same_output(
    supply_id: UUID,
    source: SupplyStatus,
    reason: str,
    now: datetime,
    ...
    command = _command(supply_id=supply_id, reason=reason)
    triggered_by = ActorId(triggered_by_uuid)
    first = mark_supply_recovering.decide(
        state=state, command=command, now=now, triggered_by=triggered_by
    )
    second = mark_supply_recovering.decide(
        state=state, command=command, now=now, triggered_by=triggered_by
    )
    assert first == second
