"""Property-based tests for `retire_language_model.decide` (Agent BC).

Complements the example-based `test_retire_language_model_decider.py`
with universal claims across generated inputs. The decider is a pure
FSM terminal

    (state, command, now) -> list[LanguageModelRetired]
...
Load-bearing properties:
  ...
  - The source-state partition is total over `LanguageModelStatus`:
    every status in `{Approved, RetirementAnnounced}` emits exactly
    one `LanguageModelRetired` (language_model_id=state.id,
    occurred_at=now); every other status raises
    `LanguageModelCannotRetireError` carrying the current status, so
    a future status value cannot silently fall through.
  - A valid reason threads through trimmed; None stays None.
...
_DISALLOWED_SOURCES = tuple(
    s for s in LanguageModelStatus if s not in frozenset(_RETIRABLE_SOURCES)
)
...
def test_retire_with_none_state_always_raises_not_found(
    ...
    with pytest.raises(LanguageModelNotFoundError) as exc:
        decide(
            state=None,
            command=RetireLanguageModel(language_model_id=language_model_id),
            now=now,
        )
...
@given(
    ...
    source=st.sampled_from(_RETIRABLE_SOURCES),
    now=aware_datetimes(),
)
def test_retire_from_permitted_source_emits_single_event(
    language_model_id: UUID,
    source: LanguageModelStatus,
    now: datetime,
) -> None:
    """Every permitted source emits exactly one LanguageModelRetired with state.id."""
    events = decide(
    ...
    assert events == [
        LanguageModelRetired(language_model_id=language_model_id, reason=None, occurred_at=now)
    ]
...
def test_retire_from_disallowed_source_always_raises_cannot_retire(
    ...
    with pytest.raises(LanguageModelCannotRetireError) as exc:
        decide(
            state=_language_model(language_model_id=language_model_id, status=source),
            command=RetireLanguageModel(language_model_id=language_model_id),
            now=now,
...
def test_retire_threads_reason_through_trimmed(
    language_model_id: UUID,
    source: LanguageModelStatus,
    reason: str,
    now: datetime,
    ...
    events = decide(
        state=_language_model(language_model_id=language_model_id, status=source),
        command=RetireLanguageModel(language_model_id=language_model_id, reason=reason),
        now=now,
    )
...
def test_retire_uses_state_id_not_command_language_model_id(
    state_id: UUID,
    command_id: UUID,
    now: datetime,
) -> None:
    ...
    events = decide(
        ...
        command=RetireLanguageModel(language_model_id=command_id),
        now=now,
    )
...
@pytest.mark.unit
@given(language_model_id=st.uuids(), now=aware_datetimes())
def test_retire_is_pure_same_input_same_output(language_model_id: UUID, now: datetime) -> None:
