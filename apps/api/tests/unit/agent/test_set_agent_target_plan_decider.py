"""Pure-decider tests for the `set_agent_target_plan` slice."""
...
def test_sets_target_plan_from_unset_in_each_allowed_source_state(status: AgentStatus) -> None:
    ...
    events = decide(
        state=agent,
        command=SetAgentTargetPlan(agent_id=agent.id, target_plan_id=_PLAN_A),
        now=_NOW,
    )
    assert len(events) == 1
    assert isinstance(events[0], AgentTargetPlanSet)
...
def test_changes_target_plan_to_a_different_plan() -> None:
    ...
    events = decide(
        state=agent,
        command=SetAgentTargetPlan(agent_id=agent.id, target_plan_id=_PLAN_B),
        now=_NOW,
    )
    assert len(events) == 1
...
def test_clears_target_plan_to_none() -> None:
    ...
    events = decide(
        state=agent,
        command=SetAgentTargetPlan(agent_id=agent.id, target_plan_id=None),
        now=_NOW,
    )
    assert len(events) == 1
...
def test_not_found_when_state_is_none() -> None:
    with pytest.raises(AgentNotFoundError):
        decide(
            state=None,
            command=SetAgentTargetPlan(agent_id=uuid4(), target_plan_id=_PLAN_A),
            now=_NOW,
        )
