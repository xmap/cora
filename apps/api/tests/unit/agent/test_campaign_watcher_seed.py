"""Unit tests for the CampaignWatcher Agent bootstrap seed.

CampaignWatcher is the ninth seeded agent and a deterministic flag-only watcher:
no prompt template and a Rule brain (`CampaignWatcher:v1`) (it is rule-based, a periodic
staleness comparison over Held campaigns, never builds an LLM). These tests pin
that shape alongside the shared seed scaffolding.
"""
...
@pytest.mark.unit
async def test_seed_creates_campaign_watcher_at_pinned_id() -> None:
    kernel = _kernel()
    await seed_campaign_watcher_agent(kernel)

    agent = await load_agent(kernel.event_store, CAMPAIGN_WATCHER_AGENT_ID)
    assert agent is not None
    assert agent.id == CAMPAIGN_WATCHER_AGENT_ID
    assert agent.name.value == CAMPAIGN_WATCHER_AGENT_NAME
    assert agent.kind.value == CAMPAIGN_WATCHER_AGENT_KIND
    assert agent.version.value == CAMPAIGN_WATCHER_AGENT_VERSION
...
async def test_seed_is_deterministic_no_prompt_rule_brain() -> None:
    ...
    assert agent.brain == BrainRef.for_rule("CampaignWatcher:v1")
...
async def test_seed_creates_co_registered_actor() -> None:
    ...
    actor = await load_actor(kernel.event_store, CAMPAIGN_WATCHER_AGENT_ID)
    assert actor is not None
    assert actor.id == CAMPAIGN_WATCHER_AGENT_ID
...
@pytest.mark.unit
async def test_campaign_watcher_id_distinct_from_all_prior_agents() -> None:
    """The nine seeded agents share the UUID-range scheme but must NOT collide."""
    ...
    assert CAMPAIGN_WATCHER_AGENT_ID not in prior
