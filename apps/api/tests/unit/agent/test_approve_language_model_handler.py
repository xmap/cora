"""Application-handler tests for the `approve_language_model` slice."""
...
from cora.agent.features import approve_language_model
from cora.agent.features.approve_language_model import ApproveLanguageModel
...
_LANGUAGE_MODEL_ID = UUID("01900000-0000-7000-8000-00000000d001")
_GENESIS_EVENT_ID = UUID("01900000-0000-7000-8000-00000000d002")
_APPROVE_EVENT_ID = UUID("01900000-0000-7000-8000-00000000d003")
...
@pytest.mark.unit
async def test_handler_approves_a_defined_language_model() -> None:
    store = InMemoryEventStore()
    await _seed_defined_language_model(store)
    deps = _build_deps(event_store=store)
    handler = approve_language_model.bind(deps)
    await handler(
        ApproveLanguageModel(language_model_id=_LANGUAGE_MODEL_ID),
        principal_id=_PRINCIPAL_ID,
        correlation_id=_CORRELATION_ID,
    )
    ...
    assert events[-1].payload["language_model_id"] == str(_LANGUAGE_MODEL_ID)
...
@pytest.mark.unit
async def test_handler_raises_cannot_approve_when_already_approved() -> None:
    store = InMemoryEventStore()
    await _seed_defined_language_model(store)
    approved = LanguageModelApproved(language_model_id=_LANGUAGE_MODEL_ID, occurred_at=_T1)
    await store.append(
        stream_type="LanguageModel",
        stream_id=_LANGUAGE_MODEL_ID,
        expected_version=1,
        events=[
            to_new_event(
                ...
                event_id=UUID("01900000-0000-7000-8000-00000000d099"),
