"""Unit tests for the vendor-API RunDebriefer Agent bootstrap seed."""
...
@pytest.mark.unit
async def test_seed_creates_run_debriefer_external_at_pinned_id() -> None:
    kernel = _kernel()
    await seed_run_debriefer_external_agent(kernel)

    agent = await load_agent(kernel.event_store, RUN_DEBRIEFER_EXTERNAL_AGENT_ID)
    assert agent is not None
    assert agent.id == RUN_DEBRIEFER_EXTERNAL_AGENT_ID
    assert agent.name.value == RUN_DEBRIEFER_EXTERNAL_AGENT_NAME
    assert agent.kind.value == RUN_DEBRIEFER_EXTERNAL_AGENT_KIND
    assert agent.version.value == RUN_DEBRIEFER_EXTERNAL_AGENT_VERSION
...
async def test_seed_creates_co_registered_actor() -> None:
    ...
    actor = await load_actor(kernel.event_store, RUN_DEBRIEFER_EXTERNAL_AGENT_ID)
    assert actor is not None
    assert actor.id == RUN_DEBRIEFER_EXTERNAL_AGENT_ID
...
async def test_seed_is_resilient_under_pre_existing_actor_only_stream() -> None:
    ...
    kernel = _kernel()
    await seed_run_debriefer_external_agent(kernel)
    await seed_run_debriefer_external_agent(kernel)  # second seed: no-op via ConcurrencyError
...
@pytest.mark.unit
async def test_run_debriefer_external_id_distinct_from_its_siblings() -> None:
    ...
    from cora.agent.seed_run_debriefer_local import RUN_DEBRIEFER_LOCAL_AGENT_ID

    assert RUN_DEBRIEFER_EXTERNAL_AGENT_ID != RUN_DEBRIEFER_AGENT_ID
    assert RUN_DEBRIEFER_EXTERNAL_AGENT_ID != RUN_DEBRIEFER_LOCAL_AGENT_ID
