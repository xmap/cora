"""Unit tests for the local (in-house) CautionDrafter Agent bootstrap seed."""
...
@pytest.mark.unit
async def test_seed_creates_caution_drafter_local_at_pinned_id() -> None:
    kernel = _kernel()
    await seed_caution_drafter_local_agent(kernel)

    agent = await load_agent(kernel.event_store, CAUTION_DRAFTER_LOCAL_AGENT_ID)
    assert agent is not None
    assert agent.id == CAUTION_DRAFTER_LOCAL_AGENT_ID
    assert agent.name.value == CAUTION_DRAFTER_LOCAL_AGENT_NAME
    assert agent.kind.value == CAUTION_DRAFTER_LOCAL_AGENT_KIND
    assert agent.version.value == CAUTION_DRAFTER_LOCAL_AGENT_VERSION
...
async def test_seed_creates_co_registered_actor() -> None:
    ...
    actor = await load_actor(kernel.event_store, CAUTION_DRAFTER_LOCAL_AGENT_ID)
    assert actor is not None
    assert actor.id == CAUTION_DRAFTER_LOCAL_AGENT_ID
...
async def test_seed_is_resilient_under_pre_existing_actor_only_stream() -> None:
    ...
    kernel = _kernel()
    await seed_caution_drafter_local_agent(kernel)
    await seed_caution_drafter_local_agent(kernel)  # second seed: no-op via ConcurrencyError
...
@pytest.mark.unit
async def test_caution_drafter_local_id_distinct_from_its_siblings() -> None:
    ...
    assert CAUTION_DRAFTER_LOCAL_AGENT_ID != CAUTION_DRAFTER_AGENT_ID
    assert CAUTION_DRAFTER_LOCAL_AGENT_ID != CAUTION_DRAFTER_EXTERNAL_AGENT_ID
