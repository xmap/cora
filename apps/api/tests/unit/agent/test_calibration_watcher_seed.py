"""Unit tests for the CalibrationWatcher Agent bootstrap seed.

CalibrationWatcher is the seventh seeded agent and (with ClearanceWatcher) a
deterministic flag-only watcher: no prompt template and a Rule brain (`CalibrationWatcher:v1`) (it
is rule-based, a periodic staleness comparison, never builds an LLM). These tests
...
@pytest.mark.unit
async def test_seed_creates_calibration_watcher_at_pinned_id() -> None:
    kernel = _kernel()
    await seed_calibration_watcher_agent(kernel)

    agent = await load_agent(kernel.event_store, CALIBRATION_WATCHER_AGENT_ID)
    assert agent is not None
    assert agent.id == CALIBRATION_WATCHER_AGENT_ID
    assert agent.name.value == CALIBRATION_WATCHER_AGENT_NAME
    assert agent.kind.value == CALIBRATION_WATCHER_AGENT_KIND
    assert agent.version.value == CALIBRATION_WATCHER_AGENT_VERSION
...
async def test_seed_is_deterministic_no_prompt_rule_brain() -> None:
    ...
    assert agent.brain == BrainRef.for_rule("CalibrationWatcher:v1")
...
async def test_seed_creates_co_registered_actor() -> None:
    ...
    actor = await load_actor(kernel.event_store, CALIBRATION_WATCHER_AGENT_ID)
    assert actor is not None
    assert actor.id == CALIBRATION_WATCHER_AGENT_ID
...
@pytest.mark.unit
async def test_calibration_watcher_id_distinct_from_all_prior_agents() -> None:
    """The seven seeded agents share the UUID-range scheme but must NOT collide."""
    ...
    assert CALIBRATION_WATCHER_AGENT_ID not in prior
