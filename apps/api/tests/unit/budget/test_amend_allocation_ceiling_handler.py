"""Application-handler tests for the `amend_allocation_ceiling` slice."""
...
from cora.budget.features.amend_allocation_ceiling import AmendAllocationCeiling
...
_AMEND_EVENT_ID = UUID("01900000-0000-7000-8000-00000000e002")
...
@pytest.mark.unit
async def test_handler_amends_ceiling_of_a_granted_allocation() -> None:
    ...
    await handler(
        AmendAllocationCeiling(allocation_id=_ALLOCATION_ID, ceiling_usd=18000.0),
        principal_id=_PRINCIPAL_ID,
        correlation_id=_CORRELATION_ID,
    )
    events, version = await store.load("Allocation", _ALLOCATION_ID)
    assert version == 2
    assert events[-1].event_type == "AllocationCeilingAmended"
...
async def test_handler_same_ceiling_appends_nothing() -> None:
    ...
    handler = amend_allocation_ceiling.bind(deps)
    await handler(
        AmendAllocationCeiling(allocation_id=_ALLOCATION_ID, ceiling_usd=25000.0),
        principal_id=_PRINCIPAL_ID,
        correlation_id=_CORRELATION_ID,
    )
    events, version = await store.load("Allocation", _ALLOCATION_ID)
...
@pytest.mark.unit
async def test_handler_raises_cannot_amend_after_void() -> None:
    store = InMemoryEventStore()
