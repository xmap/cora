"""Property-based tests for `start_visit.decide` (Trust BC, Visit aggregate).

Complements the example-based `test_start_visit_decider.py` with
universal claims across generated inputs. The decider is a pure
single-source FSM transition

    (state, command, now) -> list[VisitStarted]
...
Load-bearing properties:
  ...
  - The source-state partition is total over `VisitStatus`: only
    `Arrived` emits exactly one `VisitStarted` (visit_id=state.id,
    occurred_at=now); every other status raises `VisitCannotStartError`
...
_STARTABLE_SOURCES = (VisitStatus.ARRIVED,)
_DISALLOWED_SOURCES = tuple(s for s in VisitStatus if s not in frozenset(_STARTABLE_SOURCES))
...
def test_start_with_none_state_always_raises_not_found(
    visit_id: UUID,
    now: datetime,
) -> None:
...
@pytest.mark.unit
@given(visit_id=st.uuids(), now=aware_datetimes())
def test_start_from_arrived_emits_single_event(visit_id: UUID, now: datetime) -> None:
    """Arrived is the only startable source; emits one VisitStarted."""
    events = decide(
        state=_visit(visit_id=visit_id, status=VisitStatus.ARRIVED),
        command=StartVisit(visit_id=visit_id),
        now=now,
    )
    assert events == [VisitStarted(visit_id=visit_id, occurred_at=now)]
...
def test_start_from_disallowed_source_always_raises_cannot_start(
    ...
    """Any source other than Arrived raises, carrying the current status."""
    with pytest.raises(VisitCannotStartError) as exc:
        decide(
...
def test_start_uses_state_id_not_command_visit_id(
    state_visit_id: UUID,
    ...
    events = decide(
        state=_visit(visit_id=state_visit_id, status=VisitStatus.ARRIVED),
        command=StartVisit(visit_id=command_visit_id),
        now=now,
    )
...
@pytest.mark.unit
@given(visit_id=st.uuids(), now=aware_datetimes())
def test_start_is_pure_same_input_same_output(visit_id: UUID, now: datetime) -> None:
