...
@st.composite
def _single_device_intervals(draw: st.DrawFn, *, weighted: bool = False) -> list[ServedInterval]:
    ...
    for i in range(count):
        ...
        intervals.append(_iv(f"c{i}", float(start), float(start + duration), weight=float(weight)))
    return intervals


@st.composite
def _multi_device_intervals(draw: st.DrawFn, *, weighted: bool = False) -> list[ServedInterval]:
    count = draw(st.integers(min_value=0, max_value=6))
    intervals: list[ServedInterval] = []
    for i in range(count):
        start = draw(st.integers(min_value=0, max_value=20))
