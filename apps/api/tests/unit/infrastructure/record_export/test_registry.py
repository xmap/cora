...
See `cora.infrastructure.record_export._registry` for the design: eight
entries, six resolved from a `*LogbookOpened` envelope's `kind` and two
(`heartbeat`, `probe`) declared explicitly because they have no envelope.
"""
...
_DECLARED_KINDS = ("heartbeat", "probe")


def test_registry_has_eight_entries_not_six() -> None:
...
def test_order_by_uses_sampled_at_on_exactly_the_four_tables_that_have_it() -> None:
    ...
    assert without_sampled_at == {"verdict", "inference", "heartbeat", "probe"}
